...
namespace SVfit_namespace
{
  ...
  double logGaussianNd(const T1& residual, const T2& cov)
  {
    ...
    if ( cov.Diagonal().Dim() != numDimensions ) {
      ...
      std::cout<< " Dimension of covariance matrix = "
    ...
    if ( det == 0. ) {
      //edm::LogError ("logGaussianNd")
      std::cout<< " Cannot invert " << numDimensions << "x" << numDimensions
    ...
    if ( flag == false ) {
      //edm::LogError ("logGaussianNd")
      std::cout<< " Failed to invert covariance matrix, error flag = " << flag << " !!"<<std::endl;
      return std::numeric_limits<float>::min();
    }
  ...
  double logGaussianNdInvertedCovariance(
    ...
    if ( covInverse.Diagonal().Dim() != numDimensions ) {
      //edm::LogError ("logGaussianNd")
      std::cout<< " Dimension of covariance matrix = " << covInverse.Diagonal().Dim()
