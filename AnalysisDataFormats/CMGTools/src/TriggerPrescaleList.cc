...
cmg::TriggerPrescaleList::TriggerPrescaleList(const edm::Handle<edm::TriggerResults> & handle) :
    prescaleValues_(), 
    triggerResults_(edm::RefProd<edm::TriggerResults>(handle).refCore()),
    triggerNames_(0)
{
    prescaleValues_.resize(handle->size(),UnSet(int));
}

int cmg::TriggerPrescaleList::getPrescaleForIndex(int index) const {
...
int cmg::TriggerPrescaleList::getPrescaleForName(const std::string & name, bool prefixOnly) const {
...
void cmg::TriggerPrescaleList::addPrescaledTrigger(int index, int prescale) {
    if (unsigned(index) >= triggerResults().size()) throw cms::Exception("InvalidReference", "Index out of bounds");
