...
//    int    BinForYields[] = {i+1, i+1, i+1, i+1, i+1, i+1};
      ...
      isUncert=false; sprintf(pFileB,"jesdown  lnN ");for(int c=0;c<NChannels;c++){for(int sc=0;sc<NsubChannels;sc++){stYields tmpSyst = Efficiency(File,subChannelsDir[sc] ,string("/") + Channels[c] + "_" + "jer" + HistoName[c] ,BinForYields[c]); stYields tmp = Efficiency(File,subChannelsDir[sc] ,string("/") + Channels[c] + "_" + HistoName[c] ,BinForYields[c]);Unc=tmp.Rate==0.0 ? 1.0 : tmpSyst.Rate/tmp.Rate; isUncert|=(Unc!=1.0); sprintf(pFileB,"%s%7f   ",pFileB,Unc);}}if(isUncert)fprintf(pFile,"%s\n",pFileB); 
