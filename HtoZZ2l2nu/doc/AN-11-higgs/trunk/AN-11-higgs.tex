...
\RCS$Id: AN-11-higgs.tex,v 1.25 2011/09/13 14:51:54 psilva Exp $
%%%%%%%%%%%%% ptdr definitions %%%%%%%%%%%%%%%%%%%%%
...
\begin{tabular}{lccccc} \hline
  & $|M-M_Z|<$15~GeV/c$^2$    & \multirow{2}{*}{$\Delta\eta$} & \multirow{2}{*}{inv. Mass} & \multirow{2}{*}{Lep.+Jet Veto} \\
...
The procedure is expected to have optimal resolution on the lepton reconstruction as described in~\cite{Abazov:2010mk}.
...
For the VBF category a single working point is chosen: \RMET$>$50~GeV.
...
%%% Pileup
%%%
\subsection{Description of the FastJet clustering method}
...
Table~\ref{} \texttt{FIXME: point to the cut flow table} shows the
expecred number of events surviving the \RMET cut. The di-boson
processes and the \ttbar are the dominant remaining backgrounds at
this stage of the selection.  

To further enhance the signal to backround ratio we setup a
multivariate analysis exploiting mainly angular variables for the
rejection of the irreducible di-boson processes.
In the current implementation the variables used for the selection
are:
\begin{itemize}
\item $\Delta \phi (lead-\ell, \ell\ell)$: angle between the leading
  lepton and the di-lepton system;
\item $\Delta R (\ell, \ell)$: angular distance between the two leptons;
\item  $\sum M_T$: sum of the individual transverse masses of each lepton and E miss.
\end{itemize}
Figure~\ref{fig:angles} shows the observables used to implement the discriminant.
\begin{figure}[htp]
\begin{center}
\includegraphics[width=0.3\textwidth]{img/angles}
\caption{Angular variables used in the implementation of the
  likelihood discriminant. The plots refer to an integrated luminosity
of 1.1~1/fb.}
\label{fig:angles}
\end{center}
\end{figure}

The distribution of the input variables for all the backgrounds and
for the different mass hipothesis for the signal in the di-muon final
state are shown before the \RMET cut in Figure~\ref{} and after the
loose selection cut in Figure~\ref{fig:input_redMET}.
...
\caption{Distribution of the input variables used to build the
  likelihood discriminant for data and MC in the di-muon chanels. The
  plots show the events after the \RMET selection (loose working
  point). The plots refer to an integrated luminosity
of 1.1~1/fb. \texttt{FIXME: out-of-date, just a placeholder}}
...
\texttt{FIXME: add the di-electron plots}

The likelihood discriminant:
\begin{equation}
L.R. = \frac{L_{s}}{L_{s}+L_{b}};
\label{eq:likelihood}
\end{equation}
where:
\begin{equation}
L = \prod_{variables} pdf.
\label{eq:likelihood}
\end{equation}
has been trained against the di-boson irreducible backgorunds and the
signal (including both the gg-fusion and VBF contributions to the ZZ
and WW final states). For this task the MC samples are split in two to
have two statistically independent samples for the training and the
evaluation of the performance of the discriminant.
The training is performed in different bins of jet multiplicity to
further exploit the signal signature enhancing the signal significance
in the cleanest bins.

The output of the discriminant in the different bins of number of jets
is shown in Fig.~\ref{fig:likelihoodout_dimu} for data and MC
including all backgrounds and the signal for an Higgs mass of
300~\GeV.

\begin{figure}[htp]
\begin{center}
\includegraphics[width=0.3\textwidth]{img/cStack_TMVALikelihood_j0}
\includegraphics[width=0.3\textwidth]{img/cStack_TMVALikelihood_j1}
\includegraphics[width=0.3\textwidth]{img/cStack_TMVALikelihood_j2}
\caption{Output of the likelihood discriminant for data and MC in the
  di-muon final state. The different plots refer to different bins of
  jet multiplicity: 0 jet (left), 1 jet (center) and more than 1 jet
  (right). The plots refer to an integrated luminosity
of 1.1~1/fb.\texttt{FIXME: out-of-date, just a placeholder}}
...
The backgrounds can be further reduced by applying the third lepton-veto and by requiring missing transverse energy (\RMET>57.4 GeV/c).
...
As it can be seen from the same table, the dominant background for the VBF selection are the $gg \rightarrow VV \rightarrow >1l \geq 1nu$ events
...
H(200)$\rightarrow$ VV & 3.29E-01+-1.39E-02 & 4.41E-01+-7.15E-02 \\
H(300)$\rightarrow$ VV & 4.08E-01+-2.09E-02 & 4.29E-01+-2.16E-02 \\
H(400)$\rightarrow$ VV & 2.46E-01+-9.47E-03 & 2.38E-01+-8.74E-03 \\
H(500)$\rightarrow$ VV & 1.33E-01+-1.08E-02 & 1.21E-01+-1.22E-02 \\
H(600)$\rightarrow$ VV & 7.01E-02+-2.26E-03 & 6.51E-02+-2.22E-03 \\
ZZ         & 2.48E-02+-9.99E-03 & 1.97E-02+-9.54E-03 \\
WW         & 1.51E-02+-1.51E-02 & 0.00E+00+-0.00E+00 \\
WZ         & 3.63E-02+-1.96E-02 & 6.18E-02+-2.57E-02 \\
Single top & 2.49E-02+-2.35E-02 & 4.95E-02+-4.39E-02 \\
$t\bar{t}$ & 3.10E-01+-2.22E-01 & 5.80E-02+-5.80E-02 \\
W+jets     & 0.00E+00+-0.00E+00 & 0.00E+00+-0.00E+00 \\
Z/$\gamma^{*}$+jets$\rightarrow$ ll &  3.88E-01+-2.82E-01 & 5.13E-02+-3.75E-02 \\ \hline
Total expected    & 7.99E-01+-5.72E-01 & 2.40E-01+-1.75E-01 \\\hline
Data              & 1.00E+00+-1.00E+00 & 0.00E+00+-0.00E+00 \\\hline
\end{tabular}
\end{center}
\end{table}
