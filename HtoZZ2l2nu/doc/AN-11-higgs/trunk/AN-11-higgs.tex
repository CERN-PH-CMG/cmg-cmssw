...
\RCS$Id: AN-11-higgs.tex,v 1.4 2011/08/23 16:40:07 psilva Exp $
%%%%%%%%%%%%% ptdr definitions %%%%%%%%%%%%%%%%%%%%%
...
%%% PRE-SELECTION
\item[Pre-selection filters] are applied to minimize the contamination from beam-gas interactions, beam-halo, calorimeter noise, etc.
...
where each $I$ represents the sum of the transverse momenta of the photons, neutral hadrons and charged hadrons
reconstructed inside the isolation cone.
...
can be estimated using the $\rho$-parameter computed using the fast jet algorithm~\cite{}.
...
In this study we choose however not to perform any correction of this type,
as events with large fluctuations of the $\rho$ parameter can lead to overcorrections of the lepton isolation leading to an increase
of the contamination from non-isolated lepton candidates.

\FIXME{Needs update}
\begin{figure}[htp]
\begin{center}
%\includegraphics[width=0.49\textwidth]{img/muonIsolationAverage}
%\includegraphics[width=0.49\textwidth]{img/muonIsolationRMS}
\caption{
Isolation and $\rho$ profiles in simulated $Z\rightarrow \mu\mu$ events.
