...
void histosForLimit2012(long sm=0, long mass=0){
  //sm=0,1,2,3,4
  ...
  analysis->MTcat_=1;
  analysis->SMcat_=-1;
  analysis->extrasel_+=TString("*(categorySM2012==")+sm+")";
  ...
  TH1F* W = 0;
  if(sm==0||sm==10||sm==1||sm==11) W = analysis->getWJetsInc();
  if(sm==2 || sm==3) W = analysis->getWJetsTauIsoSM();
  ...
  TH1F* QCD = 0;
  if(sm==0||sm==10||sm==1||sm==11)QCD=analysis->getQCDInc();
  if(sm==2||sm==3)QCD=analysis->getQCDIsoSM();
