...
void DijetMass_chiyoung_mu10(){
  ...
  */
  ...
  for(int i=0; i<hDijetMass->GetNbinsX(); i++){
    ...
    if(n>=1.0){
      ...
      hQCD_Xsec->SetBinError(i+1, err/(dm*lumi)*5);

    cout << "n = " << n/(dm*lumi*scaleFactor) << "err = " << err/(dm*lumi*sqrt(scaleFactor)) << endl;
  ...
  for(int i=0; i<hDijetMass->GetNbinsX(); i++){
    ...
    if(n>=1.0){
      ...
      hJESplus_temp2->SetBinError(i+1, err/(dm*lumi)*5);
    }
    else{
  ...
  for(int i=0; i<hDijetMass->GetNbinsX(); i++){
    ...
    if(n>=1.0){
      ...
      hJESminus_temp2->SetBinError(i+1, err/(dm*lumi)*5);
    }
    else{
  ...
  // QCD Fit -- fit to qcd
  TF1 *f_qcd = new TF1("fit_qcd",fitQCD,890.0,3558.0,4); 
  ...
  TF1 *f_qcd_up = new TF1("fit_qcd_up",fitQCD,890.0,3558.0,4); 
  gStyle->SetOptFit(1111); 
  ...
  TF1 *f_qcd_down = new TF1("fit_qcd_down",fitQCD,890.0,3558.0,4); 
  gStyle->SetOptFit(1111); 
  ...
  for(i=0;i<hDijetMass->GetNbinsX();i++)
    {
      ...
      else if (n<25 && mass<=890 && mass>3854 && n>0)
	{
	  nl = n-0.5*TMath::ChisquareQuantile(a,2*n);
	  nh = 0.5*TMath::ChisquareQuantile(1-a,2*(n+1))-n;
	  veyl[i] = nl/(lumi*dm);
  ...
  // Fit to data    
  TF1 *fit = new TF1("fit",fitQCD1,890.0,3858.0,4); // 4 Par. Fit
  gStyle->SetOptFit(1111); 
  ...
  fit->SetLineWidth(2);
  fit->SetLineColor(4);
  g->Fit("fit","","",890.0,3858.0);	
    
  //Alternate Fits 4 parameter
  TF1 *f_4par = new TF1("fit_4par",fitQCD,890.0,3858.0,4); // 4 Par. Fit
  gStyle->SetOptFit(1111); 
  ...
  TF1 *f_3par = new TF1("fit_3par",fitQCD2,890.0,3858.0,3); // 3 Par. Fit
  gStyle->SetOptFit(1111);
  ...
  g->GetXaxis()->SetRangeUser(700,3600.0);
  ...
  g2->GetXaxis()->SetRangeUser(700,3600);
  g2->GetYaxis()->SetRangeUser(0.000005,50);
  g2->Draw("APZ");
  g2->Fit("fit","","sames",890.0,3858.0);

  TString status_default= gMinuit->fCstatu.Data();
  g2->Fit("fit_4par","+","sames",890.0,3858.0);
  TString status_4par= gMinuit->fCstatu.Data();
  g2->Fit("fit_3par","+","sames",890.0,3858.0);
  ...
  //    hJESminus->SetLineColor(5);
  htmp->GetXaxis()->SetRangeUser(700,3600);
  htmp->SetFillColor(5);
  htmp->SetLineColor(1);
  htmp->SetLineWidth(1); 
  htmp->SetLineColor(5);
  ...
  c3->SetLogy();
  h->Draw("C");
  h->GetXaxis()->SetRangeUser(700,3600);
  ...
  h2->Draw("C");  
  h2->SetTitle("");
  h2->GetXaxis()->SetRangeUser(700,3600);
  ...
  //   htmp2->Draw("same");
  gratio->Draw("PZsame");
  TLine *l = new TLine(700, 1., 3600., 1.);
  l->Draw("same");
  ...
  TH1F *h_zprime1 = (TH1F*)hDijetMass->Clone("h_zprime1");
  TH1F *h_zprime2 = (TH1F*)hDijetMass->Clone("h_zprime2");
  ...
  unsigned int qbin1 = (zprime1 / 100) -5;
  unsigned int qbin2 = (zprime2 / 100) -5;
  unsigned int sbin1 = (string1 / 100) -5;
  unsigned int sbin2 = (string2 / 100) - 5;
   
  std::vector<double> v_string1, v_string1_mjj, v_string2, v_string2_mjj, vzprime1, vzprime1_mjj, vzprime2, vzprime2_mjj, v_string1_2, v_string2_2, vzprime1_2, vzprime2_2, vzprime1_3, vzprime2_3;
  ...
  for(int i=0;i<hDijetMass->GetNbinsX();i++){
    ...
    if(mass>0.6*zprime1 && mass<1.26*zprime1){
      vzprime1.push_back(QstarBinnedProb(mass,zprime1) * zprime_newcut[qbin1]*fact / dm);
      vzprime1_2.push_back(((QstarBinnedProb(mass,zprime1) * zprime_newcut[qbin1]*fact / dm)+fitt)/fitt);
      vzprime1_3.push_back(((QstarBinnedProb(mass,zprime1) * zprime_newcut[qbin1]*fact / dm))/fitt);
      vzprime1_mjj.push_back(mass);
      
    }

    if(mass>0.6*zprime2 && mass<1.28*zprime2){
      vzprime2.push_back(QstarBinnedProb(mass,zprime2) * zprime_newcut[qbin2]*fact / dm);
      vzprime2_2.push_back(((QstarBinnedProb(mass,zprime2) * zprime_newcut[qbin2]*fact / dm)+fitt)/fitt); 
      vzprime2_3.push_back(((QstarBinnedProb(mass,zprime2) * zprime_newcut[qbin2]*fact / dm))/fitt); 
      vzprime2_mjj.push_back(mass);
    }
  }

  const unsigned size1 = vzprime1.size();
  ...
  for(int i=0; i<size1; i++){
    ...
    q1_2[i] = vzprime1_2[i];
    q1_3[i] = vzprime1_3[i];
    mass1[i] = vzprime1_mjj[i];
  }
  const unsigned size2 = vzprime2.size();
  ...
  TGraph* gr_zprime1 = new TGraph(size1, mass1, q1);  
  TGraph* gr_zprime2 = new TGraph(size2, mass2, q2);
  TGraph* gr_string1 = new TGraph(size3, mass3, s1);
  TGraph* gr_string2 = new TGraph(size4, mass4, s2);
    
  TGraph* gr_zprime1_2 = new TGraph(size1, mass1, q1_2);  
  TGraph* gr_zprime2_2 = new TGraph(size2, mass2, q2_2);
  TGraph* gr_string1_2 = new TGraph(size3-1, mass3, s1_2);
  TGraph* gr_string2_2 = new TGraph(size4-1, mass4, s2_2);
	
  TGraph* gr_zprime1_3 = new TGraph(size1, mass1, q1_3);  
  TGraph* gr_zprime2_3 = new TGraph(size2, mass2, q2_3);
    
  gr_string1->SetLineColor(ci_g);
  gr_string1->SetLineStyle(7);
  gr_string1->SetLineWidth(2);
  gr_zprime1->SetLineColor(2);
  gr_zprime1->SetLineStyle(5);
  gr_zprime1->SetLineWidth(2);
  gr_zprime2->SetLineColor(2);
  ...
  gr_zprime1_2->SetLineColor(2);
  gr_zprime1_2->SetLineStyle(5);
  gr_zprime1_2->SetLineWidth(2);
  gr_zprime2_2->SetLineColor(2);
  gr_zprime2_2->SetLineStyle(5);
  gr_zprime2_2->SetLineWidth(2);
  gr_string2_2->SetLineColor(ci_g);
  gr_string2_2->SetLineStyle(7);
  gr_string2_2->SetLineWidth(2);

  gr_zprime1_3->SetLineColor(2);
  gr_zprime1_3->SetLineStyle(5);
  gr_zprime1_3->SetLineWidth(2);
  ...
  TH1F *vFrame = gPad->DrawFrame(700.0,0.000001,3600.0,10.0);
  vFrame->SetTitle("");
  ...
  gr_zprime1->Draw("same");
  gr_zprime2->Draw("same");


  TPaveText *pt_c5_zprime1 = new TPaveText(0.18,0.35,0.33,0.45,"NDC");
  pt_c5_zprime1->SetFillColor(0);
  pt_c5_zprime1->SetFillStyle(0);
  pt_c5_zprime1->SetBorderSize(0);
  pt_c5_zprime1->SetTextColor(2);
  pt_c5_zprime1->AddText("Z'-> bbar (1.5 TeV)");

  TPaveText *pt_c5_zprime2 = new TPaveText(0.32,0.25,0.47,0.35,"NDC");
  pt_c5_zprime2->SetFillColor(0);
  pt_c5_zprime2->SetFillStyle(0);
  pt_c5_zprime2->SetBorderSize(0);
  pt_c5_zprime2->SetTextColor(2);
  pt_c5_zprime2->AddText("Z'-> bbar (2.3 TeV)");

  pt_c5_zprime1->Draw("sames");
  pt_c5_zprime2->Draw("sames");
  ...
  leg2->AddEntry(gr_zprime1,"Z'-> bbar","L");
  leg2->Draw("same");
  pave_fit->Draw("same");
  ...
  hDiff->GetXaxis()->SetRangeUser(700.,3600.);
  hDiff->GetYaxis()->SetRangeUser(-1.5,3.);
  hDiff->Draw("APZ");
  gr_zprime1_3->Draw("Lsame");
  gr_zprime2_3->Draw("Lsame");
  l = new TLine(700, 0.0, 3600, 0.0);
  l->SetLineStyle(2);
  l->Draw("same");
  pave->Draw("same");

  TPaveText *pt_c6_zprime1 = new TPaveText(0.20,0.3,0.35,0.4,"NDC");
  pt_c6_zprime1->SetFillColor(0);
  pt_c6_zprime1->SetTextColor(2);
  pt_c6_zprime1->SetFillStyle(0);
  pt_c6_zprime1->SetBorderSize(0);
  pt_c6_zprime1->AddText("Z'-> bbar (1.5 TeV)");

  TPaveText *pt_c6_zprime2 = new TPaveText(0.40,0.3,0.55,0.4,"NDC");
  pt_c6_zprime2->SetFillColor(0);
  pt_c6_zprime2->SetTextColor(2);
  pt_c6_zprime2->SetFillStyle(0);
  pt_c6_zprime2->SetBorderSize(0);
  pt_c6_zprime2->AddText("Z'-> bbar (2.3 TeV)");
  ...
  hPulls->GetXaxis()->SetRangeUser(700.,3600.);
  ...
  hratio->SetMarkerStyle(20);
  hratio->GetXaxis()->SetRangeUser(700.,3600.);
  hratio->GetYaxis()->SetRangeUser(0.5,6.);
  hratio->Draw("APZ");
  l->Draw("same");
	
  gr_zprime1_2->Draw("Lsame");
  gr_zprime2_2->Draw("Lsame");
   

  TPaveText *pt_c10_zprime1 = new TPaveText(0.2,0.5,0.35,0.6, "NDC");
  pt_c10_zprime1->SetFillColor(0);
  pt_c10_zprime1->SetTextColor(2);
  pt_c10_zprime1->SetFillStyle(0);
  pt_c10_zprime1->SetBorderSize(0);
  pt_c10_zprime1->AddText("Z'-> bbar (1.5 TeV)");

  TPaveText *pt_c10_zprime2 = new TPaveText(0.5,0.5,0.65,0.6,"NDC");
  pt_c10_zprime2->SetFillColor(0);
  pt_c10_zprime2->SetTextColor(2);
  pt_c10_zprime2->SetFillStyle(0);
  pt_c10_zprime2->SetBorderSize(0);
  pt_c10_zprime2->AddText("Z'-> bbar (2.3 TeV)");

  pt_c10_zprime1->Draw("sames");
  pt_c10_zprime2->Draw("sames");
  ...
  c10->SaveAs("Plots_mu30/DataMinusFitWithAllSignal.eps");
   ...
   TH1F *vFrame = p11_1->DrawFrame(700.0,0.00000001,3600.0,1.01);
   vFrame->SetTitle("");
   vFrame->SetXTitle("Dijet Mass (GeV)");
   vFrame->GetXaxis()->SetTitleSize(0.06);
   vFrame->SetYTitle("d#sigma/dm (pb/GeV)");
   ...
   leg->AddEntry(htmp,"JES Uncertainty","F");
   leg->AddEntry(gr_zprime1,"Z'->bbar","L");
 ...
 gr_zprime1->Draw("same");
  ...
  TPaveText *pt_c11_zprime1 = new TPaveText(0.20,0.30,0.35,0.45,"NDC");
  pt_c11_zprime1->SetFillColor(0);
  pt_c11_zprime1->SetFillStyle(0);
  pt_c11_zprime1->SetBorderSize(0);
  pt_c11_zprime1->SetTextColor(2);
  pt_c11_zprime1->AddText("Z'-> bbar (1.5 TeV)");

  TPaveText *pt_c11_zprime2 = new TPaveText(0.40,0.15,0.55,0.20,"NDC");
  pt_c11_zprime2->SetFillColor(0);
  pt_c11_zprime2->SetFillStyle(0);
  pt_c11_zprime2->SetBorderSize(0);
  pt_c11_zprime2->SetTextColor(2);
  pt_c11_zprime2->AddText("Z'-> bbar (2.3 TeV)");

  pt_c11_zprime1->Draw("sames");
  pt_c11_zprime2->Draw("sames");
   ...
   TH1F *vFrame2 = p11_2->DrawFrame(700.0, -2.51, 3600.0, 2.51);
   vFrame2->SetTitle("");
