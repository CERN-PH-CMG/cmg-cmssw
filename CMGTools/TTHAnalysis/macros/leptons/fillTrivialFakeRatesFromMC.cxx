...
void fillTrivialFakeRatesFromMC(int triggering=1) {
    ...
    for (int is = 0; is < nsels; ++is) {
        ...
        TH2F *FR_tight5_el_den = new TH2F(Form("%s_tight5_el_den",sels[is]),"",npt_el,ptbins_el,neta_el,etabins_el);
        TH2F *FR_tight6_mu_num = new TH2F(Form("%s_tight6_mu_num",sels[is]),"",npt_mu,ptbins_mu,neta_mu,etabins_mu);
    ...
    for (int id = 0; id < 7; ++id) { 
        ...
        if (sample == "TTJetsLep" || sample == "TTLep") {
            ...
            fillFR("FR_tight4_el", baseCutT + moreCut4 + "LepGood%d_mcMatchId == 0 && abs(LepGood%d_pdgId) == 11", "LepGood%d_mva >= 0.70", sample, 3);
            fillFR("FR_tight4_mu", baseCutT + moreCut4 + "LepGood%d_mcMatchId == 0 && abs(LepGood%d_pdgId) == 13", "LepGood%d_mva >= 0.70", sample, 3);
        }
        if (sample == "TTJetsSem" || sample == "TTJets") {
        ...
        fillFR("FR_tightB_mu",  baseCutT + "LepGood%d_mcMatchId == 0 && abs(LepGood%d_pdgId) == 13", "LepGood%d_mcMatchAny == 2 && LepGood%d_mva >= 0.70", sample, 3);
        fillFR("FR_tightA2_el", baseCutT + moreCut2 + "LepGood%d_mcMatchId == 0 && abs(LepGood%d_pdgId) == 11", "LepGood%d_mcMatchAny <  2 && LepGood%d_mva >= 0.70", sample, 3);
