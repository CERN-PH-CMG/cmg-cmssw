...
void fillFRSimple(TString comp="QCDMuPt15", int selection = 0, int selbin = 0) {
    ...
    for (int itype = 1; itype <= 13; itype += 12) {
        ...
        if (selection == 3 || selection == 13 || selection == 15) {
        ...
        for (int ipdg = 11; ipdg <= 13; ipdg += 2) {
            ...
            if (selection == 3 || selection == 13 || selection == 15) { // need different binning
            ...
            for (int ieta = 0; ieta < neta; ++ieta) {
                ...
                for (int ipt = 0; ipt < npt; ++ipt) {
                    ...
                    if (selection == -1) {
                        ...
                        tightDen = (ipdg == 11 ? "Probe_innerHits == 0 && Probe_convVeto && Probe_tightCharge > 1 &&  passEgammaTightMVA(Probe_pt,Probe_eta,Probe_tightId) && abs(Probe_dxy) < 0.0200  && abs(Probe_dz) < 0.1 && (abs(Probe_eta) < 1.4442 || abs(Probe_eta) > 1.566)" : 
                                                 "Probe_tightId && Probe_tightCharge > 0  && abs(Probe_dxy) < 0.0100*(1+(Probe_pt > 20)) && abs(Probe_dz) < 0.1");
                        tightNum = "Probe_relIso < 0.12";
                        looseNum = "";
                        looseDen = "";
                    }
