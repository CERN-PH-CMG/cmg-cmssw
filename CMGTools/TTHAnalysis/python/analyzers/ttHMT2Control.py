...
#number of PF hadrons with pt > 10, reliso < 0.1, MT < 100                        
    ...
    def makeGammaObjects(self, event):
        ...
        ## with Central Jets                                                                                                                                                                                                      
        gamma_objects25 = [ j for j in event.gamma_cleanJets if j.pt() > 25 and abs(j.eta())<self.cfg_ana.jetEta ] + event.selectedLeptons
        gamma_objects30 = [ j for j in event.gamma_cleanJets if j.pt() > 30 and abs(j.eta())<self.cfg_ana.jetEta ] + event.selectedLeptons
        ...
        gamma_objects40ja = [ j for j in event.gamma_cleanJetsAll if j.pt() > 40 and abs(j.eta())<self.cfg_ana.jetEta ]
    ...
    def makeZllObjects(self, event):
        ...
        if len(vetoLeptons)==2:
            ...
            objects40ja = [ j for j in event.cleanJets if j.pt() > 40 and abs(j.eta())<self.cfg_ana.jetEta ]
