...
def bookLepton( tree, pName, isMC=False ):
    ...
    var(tree, '{pName}_edxy'.format(pName=pName))
    var(tree, '{pName}_edz'.format(pName=pName))
    ...
    if True:
        var(tree, '{pName}_pxb1'.format(pName=pName), int)
        var(tree, '{pName}_pxlay'.format(pName=pName), int)
        var(tree, '{pName}_ehi'.format(pName=pName), int)
        var(tree, '{pName}_eho'.format(pName=pName), int)
        var(tree, '{pName}_ptRelErr'.format(pName=pName) )
        var(tree, '{pName}_segComp'.format(pName=pName) )
        var(tree, '{pName}_caloComp'.format(pName=pName) )
        var(tree, '{pName}_nStat'.format(pName=pName), int )
        var(tree, '{pName}_muTimeValid'.format(pName=pName), int )
        var(tree, '{pName}_timeIpInOut'.format(pName=pName) )
        var(tree, '{pName}_timeIpInOutErr'.format(pName=pName) )
        #var(tree, '{pName}_dEdx'.format(pName=pName) )
        #var(tree, '{pName}_dEdxErr'.format(pName=pName) )
    if isMC:
        ...
        var(tree, '{pName}_mcMatchAny2'.format(pName=pName), int)
...
def fillLepton( tree, pName, lepton ):
    ...
    if abs(lepton.pdgId())==11:
        ...
        fill(tree, '{pName}_edxy'.format(pName=pName), lepton.sourcePtr().edB() )
        fill(tree, '{pName}_edz'.format(pName=pName), lepton.sourcePtr().gsfTrack().dzError() )
        fill(tree, '{pName}_pxb1'.format(pName=pName), lepton.sourcePtr().gsfTrack().hitPattern().hasValidHitInFirstPixelBarrel() )
        fill(tree, '{pName}_pxlay'.format(pName=pName), lepton.sourcePtr().gsfTrack().hitPattern().pixelLayersWithMeasurement() )
        fill(tree, '{pName}_ehi'.format(pName=pName), lepton.sourcePtr().gsfTrack().trackerExpectedHitsInner().numberOfLostHits() )
        fill(tree, '{pName}_eho'.format(pName=pName), lepton.sourcePtr().gsfTrack().trackerExpectedHitsOuter().numberOfLostHits() )
    if abs(lepton.pdgId())==13:
        ...
        fill(tree, '{pName}_edxy'.format(pName=pName), lepton.sourcePtr().edB() )
        fill(tree, '{pName}_edz'.format(pName=pName), lepton.sourcePtr().innerTrack().dzError() )
        fill(tree, '{pName}_pxb1'.format(pName=pName), lepton.sourcePtr().innerTrack().hitPattern().hasValidHitInFirstPixelBarrel() )
        fill(tree, '{pName}_pxlay'.format(pName=pName), lepton.sourcePtr().innerTrack().hitPattern().pixelLayersWithMeasurement() )
        fill(tree, '{pName}_ehi'.format(pName=pName), lepton.sourcePtr().innerTrack().trackerExpectedHitsInner().numberOfLostHits() )
        fill(tree, '{pName}_eho'.format(pName=pName), lepton.sourcePtr().innerTrack().trackerExpectedHitsOuter().numberOfLostHits() )
        fill(tree, '{pName}_ptRelErr'.format(pName=pName), lepton.sourcePtr().innerTrack().ptError()/abs(lepton.sourcePtr().innerTrack().pt()) )
        ...
        fill(tree, '{pName}_nStat'.format(pName=pName), lepton.sourcePtr().numberOfMatchedStations() )
        fill(tree, '{pName}_muTimeValid'.format(pName=pName), lepton.sourcePtr().isTimeValid() )
        fill(tree, '{pName}_timeIpInOut'.format(pName=pName), lepton.sourcePtr().time().timeAtIpInOut )
        fill(tree, '{pName}_timeIpInOutErr'.format(pName=pName), lepton.sourcePtr().time().timeAtIpInOutErr  )
    ...
    if hasattr(lepton, 'mcMatchId'):
        ...
        fill(tree, '{pName}_mcMatchAny2'.format(pName=pName), lepton.mcMatchAny2)
...
# LEPTON
#----------
def bookTau( tree, pName, isMC=False ):
    bookParticle(tree, pName )
    var(tree, '{pName}_pdgId'.format(pName=pName), int)
    var(tree, '{pName}_charge'.format(pName=pName), int)
    var(tree, '{pName}_dxy'.format(pName=pName))
    var(tree, '{pName}_dz'.format(pName=pName))
    var(tree, '{pName}_byRawIMVA'.format(pName=pName))
    var(tree, '{pName}_byCI'.format(pName=pName), int)
    var(tree, '{pName}_byIMVA'.format(pName=pName), int)
    if isMC:
        var(tree, '{pName}_mcMatchId'.format(pName=pName), int)
    
def fillTau( tree, pName, lepton ):
    fillParticle(tree, pName, lepton )
    fill(tree, '{pName}_pdgId'.format(pName=pName), lepton.pdgId() )
    fill(tree, '{pName}_charge'.format(pName=pName), lepton.charge() )
    fill(tree, '{pName}_dxy'.format(pName=pName), lepton.dxy() )
    fill(tree, '{pName}_dz'.format(pName=pName), lepton.dz() )
    fill(tree, '{pName}_byRawIMVA'.format(pName=pName), lepton.tauID("byRawIsoMVA") )
    fill(tree, '{pName}_byCI'.format(pName=pName),   lepton.idCI )
    fill(tree, '{pName}_byIMVA'.format(pName=pName), lepton.idIMVA )
    if hasattr(lepton, 'mcMatchId'):
        fill(tree, '{pName}_mcMatchId'.format(pName=pName), lepton.mcMatchId)
