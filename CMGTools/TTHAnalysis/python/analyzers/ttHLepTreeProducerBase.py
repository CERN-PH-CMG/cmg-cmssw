...
class ttHLepTreeProducerBase( TreeAnalyzerNumpy ):
    ...
    def process(self, iEvent, event):
        ...
        ## --- INCLUSIVE LEPTONS (OPTIONAL) ---
        if hasattr(self.cfg_ana, 'doInclusiveLeptons') and self.cfg_ana.doInclusiveLeptons:
            fill(tr, 'nLep', len(event.inclusiveLeptons))
            for i in range(min(8,len(event.inclusiveLeptons))):
                fillLepton( tr, "Lep%d"%(i+1), event.inclusiveLeptons[i])
        ...
        ## --- TAUS LEPTONS (OPTIONAL) ---
        if hasattr(self.cfg_ana, 'doTaus') and self.cfg_ana.doTaus:
            fill(tr, 'nTau', len(event.selectedTaus))
            for i in range(min(4,len(event.selectedTaus))):
                fillTau( tr, "Tau%d"%(i+1), event.selectedTaus[i])
