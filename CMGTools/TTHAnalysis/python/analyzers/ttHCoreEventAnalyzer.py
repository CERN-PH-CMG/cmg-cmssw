...
class ttHCoreEventAnalyzer( Analyzer ):
    ...
    def process(self, event):
        ...
        ## with Central Jets
        objects25  = [ j for j in event.cleanJets if j.pt() > 25 and abs(j.eta())<cfg_ana.jetEta ] + event.selectedLeptons
        objects30  = [ j for j in event.cleanJets if j.pt() > 30 and abs(j.eta())<cfg_ana.jetEta ] + event.selectedLeptons
        objects40  = [ j for j in event.cleanJets if j.pt() > 40 and abs(j.eta())<cfg_ana.jetEta ] + event.selectedLeptons
        objects40j = [ j for j in event.cleanJets if j.pt() > 40 and abs(j.eta())<cfg_ana.jetEta ] 
        objects50j = [ j for j in event.cleanJets if j.pt() > 50 and abs(j.eta())<cfg_ana.jetEta ] 
        objects40j10l = [ j for j in event.cleanJets if j.pt() > 40 and abs(j.eta())<cfg_ana.jetEta ] + [ l for l in event.selectedLeptons if l.pt() > 10 ] 
        objects40j10l.sort(key = lambda obj : obj.pt(), reverse = True)

        objects40j10l5t = []
        if hasattr(event, 'selectedIsoCleanTrack'):
            objects40j10l5t = [ j for j in event.cleanJets if j.pt() > 40 and abs(j.eta())<cfg_ana.jetEta ] + [ l for l in event.selectedLeptons if l.pt() > 10 ] + [ t for t in event.selectedIsoCleanTrack ]
        ...
        if self.cfg_comp.isMC:
            ...
            genObjects40j = [j for j in event.cleanGenJets if j.pt()>40 and abs(j.eta())<cfg_ana.jetEta]
            genObjects50j = [j for j in event.cleanGenJets if j.pt()>50 and abs(j.eta())<cfg_ana.jetEta]
        ...
        objects25a  = [ j for j in event.cleanJetsAll if j.pt() > 25 and abs(j.eta())<cfg_ana.jetEta ] + event.selectedLeptons
        objects30a  = [ j for j in event.cleanJetsAll if j.pt() > 30 and abs(j.eta())<cfg_ana.jetEta ] + event.selectedLeptons
        objects40a  = [ j for j in event.cleanJetsAll if j.pt() > 40 and abs(j.eta())<cfg_ana.jetEta ] + event.selectedLeptons
        objects40ja = [ j for j in event.cleanJetsAll if j.pt() > 40 and abs(j.eta())<cfg_ana.jetEta ] 

        objects40ja10l5t = []
        if hasattr(event, 'selectedIsoCleanTrack'):
            objects40ja10l5t = [ j for j in event.cleanJetsAll if j.pt() > 40 and abs(j.eta())<cfg_ana.jetEta ] + [ l for l in event.selectedLeptons if l.pt() > 10 ] + [ t for t in event.selectedIsoCleanTrack ]
            objects40ja10l5t.sort(key = lambda obj : obj.pt(), reverse = True)

        objects40ja10l = [ j for j in event.cleanJetsAll if j.pt() > 40 and abs(j.eta())<cfg_ana.jetEta ] + [ l for l in event.selectedLeptons if l.pt() > 10 ]
