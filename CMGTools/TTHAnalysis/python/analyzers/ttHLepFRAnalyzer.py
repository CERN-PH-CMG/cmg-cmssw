...
class ttHLepFRAnalyzer( TreeAnalyzerNumpy ):
    ...
    def process(self, iEvent, event):
        ...
        fill( tr, 'nBJetLoose25',  sum([j.btag('combinedSecondaryVertexBJetTags') > 0.274 for j in event.cleanJets]) )
        fill( tr, 'nBJetMedium25', sum([j.btag('combinedSecondaryVertexBJetTags') > 0.679 for j in event.cleanJets]) )
        fill( tr, 'nBJetTight25',  sum([j.btag('combinedSecondaryVertexBJetTags') > 0.898 for j in event.cleanJets]) )
