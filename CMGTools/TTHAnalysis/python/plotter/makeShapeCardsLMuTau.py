...
parser.add_option("--asimov", dest="asimov", action="store_true", help="Asimov", default=True)
...
options.weight = True
options.final  = True
options.allProcesses  = (len(masses) > 1)

mca  = MCAnalysis(args[0],options)
cuts = CutsFile(args[1],options)
report = mca.getPlotsRaw("x", args[2], args[3], cuts.allCuts(), nodata=options.asimov)
# fix the histograms of the masses
for m in masses:
    find = "_m%d_g%03d" % (m, m/2)
    repl = "_m%d" % m
    for k,x in report.iteritems():
        if find in k:
            x.SetName(str(x.GetName()).replace(find,repl))
...
procs = []; iproc = {}
signals = []
for i,s in enumerate(mca.listSignals(allProcs=True)):
    print s
    if allyields[s] == 0: continue
    # now remove also the mass
    s = re.sub(r"_m\d+_g\d\d\d$","",s)
    if s not in procs:
        signals.append(s)
        procs.append(s); iproc[s] = -len(signals)+1 
...
def getYieldScale(mass,proc):
    if proc in signals:
        return -1
    return allyields[proc]
    
if not os.path.exists(outdir): os.mkdir(outdir)
for mass in masses:
    myyields = dict([(p, allyields[p] if p not in signals else -1) for p in  procs+['data_obs']]) 
    ...
    datacard.write("shapes S3m  * %s.input.root x_$PROCESS_m$MASS x_$PROCESS_m$MASS_$SYSTEMATIC\n" % binname)
    datacard.write("shapes S4m  * %s.input.root x_$PROCESS_m$MASS x_$PROCESS_m$MASS_$SYSTEMATIC\n" % binname)
    datacard.write('##----------------------------------\n')
