...
process.genHWW = process.genHWWZZtt.clone(cut = "pdgId == 25 && (abs(daughterRef(0).pdgId) == 24)")
process.genHZZ = process.genHWWZZtt.clone(cut = "pdgId == 25 && (abs(daughterRef(0).pdgId) == 23)")
process.genHtt = process.genHWWZZtt.clone(cut = "pdgId == 25 && (abs(daughterRef(0).pdgId) == 15)")
...
process.diBQuarkEta25Pt25 = cms.EDFilter("CandViewSelector",
    src = cms.InputTag("diBQuarks"),
    cut = cms.string("max(abs(daughter(0).eta), abs(daughter(1).eta)) < 2.5 && min(daughter(0).pt, daughter(1).pt) > 25"),
    filter = cms.bool(True),
)

if True: ## set to True to compute gen-level acceptances
    gens = [ '', 'WW', 'ZZ', 'tt' ]
    process.GEN_WW_ZZ_tt = cms.Path(process.genSeq)
    for X in gens:
        genf = process.genSeq
        if X != '': 
            genf = cms.Sequence(process.genSeq + getattr(process, 'genH'+X))
            setattr(process,'GEN_'+X, cms.Path(genf))
        setattr(process,'ACC'+X+'_2L_2010',           cms.Path(genf + process.diLepFilterGen2010))
        setattr(process,'ACC'+X+'_2L_2010SS',         cms.Path(genf + process.diLepFilterGen2010SS))
        setattr(process,'ACC'+X+'_3L_2010',           cms.Path(genf + process.diLepFilterGen2010 + process.triLepFilterGen))
        setattr(process,'ACC'+X+'_4L_2010',           cms.Path(genf + process.diLepFilterGen2010 + process.fourLepFilterGen))
        setattr(process,'ACC'+X+'_2L_2010_2B_2525',   cms.Path(genf + process.diLepFilterGen2010 +                            process.diBQuarkEta25Pt25))
        setattr(process,'ACC'+X+'_3L_2010_2B_2525',   cms.Path(genf + process.diLepFilterGen2010 + process.triLepFilterGen  + process.diBQuarkEta25Pt25))
        setattr(process,'ACC'+X+'_4L_2010_2B_2525',   cms.Path(genf + process.diLepFilterGen2010 + process.fourLepFilterGen + process.diBQuarkEta25Pt25))
        setattr(process,'ACC'+X+'_2L_2010SS_2B_2525', cms.Path(genf + process.diLepFilterGen2010SS                          + process.diBQuarkEta25Pt25))
