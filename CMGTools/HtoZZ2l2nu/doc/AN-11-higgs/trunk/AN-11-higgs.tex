...
\RCS$Id: AN-11-higgs.tex,v 1.47 2011/09/20 15:45:08 querten Exp $
%%%%%%%%%%%%% ptdr definitions %%%%%%%%%%%%%%%%%%%%%
...
{\bf Higgs mass (GeV/c$^2$)}        & {\bf 200}         & {\bf 300}         & {\bf 400}         & {\bf 500}         & {\bf 600}     \\\hline\hline
                                    &                   &                   &                   &                   &               \\\hline
\multicolumn{6}{c}{\bf$ee$ channel}\\\hline
S/B expected                        & $0.41 \pm 0.18$   & $0.51 \pm 0.23$   & $0.31 \pm 0.14$     & $0.17 \pm 0.07$     & $0.09 \pm 0.04$  \\\hline
\multicolumn{6}{c}{\bf$\mu\mu$ channel}\\\hline
S/B expected                        & $1.84 \pm 0.72$   & $1.79\pm0.65$     & $0.99\pm 0.36$      & $0.19\pm 0.37$     &  $0.27\pm 0.10$     \\\hline
\end{tabular}
\end{center}
\end{table}
...
W+jets                 &                    &  \\
Z/$\gamma^{*}$+jets$\rightarrow$ ll &  0.388 $\pm$ 0.28  & 0.051 $\pm$ 0.038 \\ \hline
Total expected                      &  0.799 $\pm$ 0.360 & 0.240 $\pm$ 0.086 \\\hline
Data                                & 1                  &  \\\hline\hline
\end{tabular}
\end{center}
\end{table}

Fig.~\ref{fig:vbf_candidate_ee} shows the event display of the di-electron event candidate selected in data.
The dilepton has an invariant mass of 92.3~GeV/c$^2$, $p_T=$152 GeV/c$^2$ and $\eta=$-1.13.
The \MET and \RMET are both 70.3~GeV and the total transverse mass of the system $M_T(Z,E_T^{miss})=$241~GeV/c$^2$.    

\begin{figure}[htp]
\begin{center}
\includegraphics[width=0.9\textwidth]{img/vbf_candidate_ee}
\caption{Event display of a di-electron candidate event with VBF-like topology.}
\label{fig:vbf_candidate_ee}
\end{center}
\end{figure}

A significant fraction of the VBF events not entering into the VBF bin, can be anyway recovered and enter into the other analysis bins (0,1 or 2 jets bin). 
One can compute the fraction of VBF events falling in the various bins of the analysis as shown in Table~\ref{tab:vbffractioninbin}.  We can observe that up to 60\% (worst for low Higgs masses) of the VBF events are actually not falling into the VBF bin.
Of course, a certain fraction of the Higs produced via gluon-gluon fusion may also pass the VBF selection and therefore enters in the VBF bin.  The fraction is obviously much smaller, as it can be seen from Table~\ref{tab:gffractioninbin}.
It could be interesting to look at a third table, showing the contribution of the VBF events into each bins.  From this last table, we can observe that the contribution of the glu-glu Higgs events into the VBF bin can go as high as 30\%.

\begin{table}[htp]
\caption{Repartition of the selected "true" VBF events into the different analysis bins.}
\label{tab:vbffractioninbin}
\begin{center}
\begin{tabular}{lccccccccc} \hline\hline
         & $\mu\mu$            & $\mu\mu$            & $\mu\mu$            & $\mu\mu$            & ee                  & ee                  & ee                  & ee                  \\
Samples  &       0 jet         & 1 jet               & 2 jets              & VBF                 & 0 jet               & 1 jet               & 2 jets              & VBF                 \\
H200     &   7.20\%            &  18.71\%            &  15.31\%            &  58.79\%            &   9.35\%            &  16.71\%            &  15.79\%            &  58.15\%            \\
H300     &  13.68\%            &  19.12\%            &  14.85\%            &  52.35\%            &  13.82\%            &  19.85\%            &  15.54\%            &  50.79\%            \\
H400     &  16.55\%            &  23.94\%            &  17.82\%            &  41.69\%            &  16.69\%            &  24.49\%            &  18.73\%            &  40.09\%            \\
H500     &  17.47\%            &  22.73\%            &  16.50\%            &  43.29\%            &  16.61\%            &  23.04\%            &  17.73\%            &  42.62\%            \\
H600     &  17.92\%            &  20.73\%            &  16.06\%            &  45.29\%            &  17.74\%            &  22.20\%            &  15.70\%            &  44.36\%            \\
\end{tabular}
\end{center}
\end{table}

\begin{table}[htp] 
\caption{Repartition of the selected "true" Gluon-Gluon events into the different analysis bins.}
\label{tab:gffractioninbin} 
\begin{center}
\begin{tabular}{lccccccccc} \hline\hline
         & $\mu\mu$            & $\mu\mu$            & $\mu\mu$            & $\mu\mu$            & ee                  & ee                  & ee                  & ee                  \\
Samples  &       0 jet         & 1 jet               & 2 jets              & VBF                 & 0 jet               & 1 jet               & 2 jets              & VBF                 \\
H200     &  38.31\%            &  39.05\%            &  18.92\%            &   3.73\%            &  37.12\%            &  26.88\%            &  34.66\%            &   1.34\%            &\\
H300     &  34.49\%            &  35.47\%            &  28.09\%            &   1.96\%            &  35.02\%            &  35.81\%            &  27.47\%            &   1.70\%            &\\
H400     &  28.81\%            &  34.42\%            &  35.03\%            &   1.75\%            &  28.91\%            &  35.03\%            &  34.22\%            &   1.84\%            &\\
H500     &  19.76\%            &  35.24\%            &  42.85\%            &   2.16\%            &  25.70\%            &  34.33\%            &  37.82\%            &   2.15\%            &\\
H600     &  23.08\%            &  33.87\%            &  40.82\%            &   2.22\%            &  21.71\%            &  33.20\%            &  42.95\%            &   2.14\%            &\\
\end{tabular} 
\end{center}
\end{table}
...
\begin{table}[htp]
\caption{Contribution of the VBF events into the signal for each bin. The numerator is the fraction of true VBF events and the denominator is the total number of Higgs events entering in that bin.} 
\label{tab:vbfcontributionintothe bin} 
\begin{center}
\begin{tabular}{lccccccccc} \hline\hline
         & $\mu\mu$            & $\mu\mu$            & $\mu\mu$            & $\mu\mu$            & ee                  & ee                  & ee                  & ee                  \\
Samples  &       0 jet         & 1 jet               & 2 jets              & VBF                 & 0 jet               & 1 jet               & 2 jets              & VBF                 \\
H200     &   4.04\%            &   9.70\%            &  15.35\%            &  77.95\%            &   4.65\%            &  10.73\%            &   8.09\%            &  89.36\%            &\\
H300     &   4.80\%            &   6.41\%            &   6.30\%            &  77.28\%            &   4.81\%            &   6.63\%            &   6.76\%            &  79.27\%            &\\
H400     &   4.15\%            &   4.99\%            &   3.70\%            &  64.32\%            &   4.12\%            &   4.95\%            &   3.92\%            &  61.83\%            &\\
H500     &   8.85\%            &   6.61\%            &   4.05\%            &  68.75\%            &   6.25\%            &   6.48\%            &   4.62\%            &  67.15\%            &\\
H600     &  11.09\%            &   8.95\%            &   5.95\%            &  76.61\%            &  11.38\%            &   9.51\%            &   5.43\%            &  76.49\%            &\\
\end{tabular}
\end{center}
\end{table}
...
We estimate the residual contamination from top (\ttbar and single top) background using the side-band of the $Z$ region,
and the un-tagged and pre-tagged samples. For each channel ($ee$ and $mu\mu$) four exclusive categories are defined
according to Tab.~\ref{tab:ttbarselregions}.

\begin{table}[htp]
\begin{center}
\caption{Selection regions for top background estimate from data. The $\Box$ ($\blacksquare$) denotes the requirement or not of the b-tagging veto.}
\label{tab:ttbarselregions}
\begin{tabular}{lcc} \hline\hline
Region                              & $M(\ell,\ell)$ GeV/c$^2$   & $b$ veto \\\hline
A                                   & $]76,106[$                 & $\Box$ \\
B                                   & $]61,76[ \cup ]106,121[$   & $\Box$ \\
C                                   & $]76,106[$                 & $\blacksquare$\\
D                                   & $]61,76[ \cup ]106,121[$   & $\blacksquare$\\\hline\hline
\end{tabular}
\end{center}
\end{table}

Let $N_t$ ($N_o$) be the total number of Top (other) events inside and outside the $Z$ mass region
and $R_{in}$ ($k_{in}$) the ratio of Top (other) events which are found inside the $Z$ mass region with respect to the total number of events.
Then, if $\varepsilon_t$ ($\varepsilon_o$) are the efficiency of the $b$-tagging veto for top (other) events,
one can write the following relations for the event yields expected in the different regions:

\begin{align}
<N_A> = \varepsilon_t R_{in} N_t + \varepsilon_o k_{in} N_o \\
<N_B> =  \varepsilon_t (1- R_{in}) N_t + \varepsilon_o (1- k_{in}) N_o \\  
<N_C> = (1-\varepsilon_t) R_{in} N_t + (1-\varepsilon_o) k_{in} N_o \\
<N_D> =  (1-\varepsilon_t) (1- R_{in}) N_t + (1-\varepsilon_o) (1-k_{in}) N_o   
\label{eq:abcdfortop}
\end{align}

Taking $R_{in}$ and $k_{in}$ from simulation idepdendently for the $\mu\mu$ and $ee$ channels,
and correcting the simulation predicted $\varepsilon_t$ and $\varepsilon_o$ using the data-driven scale factors
for the $b$-tagging efficiency which will be discussed in more detail in Sec.~\ref{subsec:systunc},
we determine $N_t$ ($N_o$) by maximizing the following likelihood:

\begin{equation}
\mathcal{L}=\displaystyle{\prod_{i=A,B,C,D}} \mathcal{P}_{oisson}(N_i, <N_i>)
\label{eq:yieldsfit}
\end{equation}

where $N_i$ ($<N_i>$) are the number of observed (expected) events in each region.


%%%
%%% Di-bosons
%%%
\subsubsection{Control region for 3 lepton events}
\label{subsubsec:threeleptoncontrol}
...
The differences found after the applying the medium selection cut for the selection for\RMET variable are summarized in Tab~\ref{tab:twuncs}.
For the VBF category the differences are quoted after the $b$-tagging veto. These two loose selections were chosen in order not to deplete the 
MC statistics used for the estimate of this uncertainty. In the tight selection region almost no single top event survives.
We assume therefore these systematic uncertainty for all the subsequent mass point selections.
This systematic is only considered for the Single Top contribution.
...
\multirow{2}{*}{Event category}     & \multicolumn{4}{c}{$\mu\mu$}              & \multicolumn{4}{c}{$ee$} \\
                                    & =0 jets & =1 jets & $\geq$ 2 jets & VBF   & =0 jets & =1 jets & $\geq$ 2 jets & VBF \\\hline
Relative uncertainty                & 1.085   & 0.925   & 1.231         & 0.645 & 1.294   & 0.916   & 0.748         & 0.820 \\\hline\hline
\end{tabular}
\end{center}
\end{table}
...
The resulting expected and observed limits can be found in the Tab.~\ref{tab:limits}, Tab.~\ref{tab:gglimits} and Tab.~\ref{tab:vbflimits} as well as in the Fig.~\ref{fig:exclusionplot}.


\begin{figure}[htp]
\begin{center}
\includegraphics[width=0.9\textwidth]{img/ExclusionPlot}
\caption{Observed (black) and Expected (grey) exclusion limit at 95\% C.L. using events falling in the 0,1,2jets (Gluon-Gluon) and VBF analysis bins.  As well as the expected limit when using only the 0,1,2 jets bins or only the VBF bin.}
...
\begin{table}[htp]
\begin{center}
\caption{Limits on the predicted SM H cross section set from the selected data.
...
Expected (MC)                         & 5.9 $\pm$ 1.9 & 2.8 $\pm$ 1.0 & 1.8 $\pm$ 0.6 &  2.6 $\pm$ 0.9 & 4.5$\pm$ 1.8 \\
Data                                  & 4.6           & 2.8           & 1.9         & 2.6          & 3.7 \\\hline\hline
\end{tabular}
\end{center}
\end{table}
...
\begin{table}[htp]
\begin{center}
\caption{Limits on the predicted SM gg$\rightarrow$H cross section set from the selected data.
The 68\% uncertainty on the expected limit is included for reference.
The uncertainty includes luminosity and MC statistics.}
\label{tab:gglimits}
\begin{tabular}{lccccc} \hline\hline
\multirow{2}{*}{$\sigma/\sigma_{SM}$} & \multicolumn{5}{c}{$m_{H^0}$ (GeV/c$^2$)} \\
                                      & 200  & 300    & 400  & 500  & 500 \\\hline
Expected (MC)                         & 11.4 $\pm$ 5.0 & 3.6 $\pm$ 1.2 & 2.2 $\pm$ 0.7 & 2.8 $\pm$ 1.1 & 4.8 $\pm$ 1.9  \\
Data                                  & 11.7          & 4.2           & 2.3        & 2.8          & 4.0 \\\hline\hline
\end{tabular}
\end{center}
\end{table}
...
\multirow{2}{*}{$\sigma/\sigma_{SM}$} & \multicolumn{5}{c}{$m_{H^0}$ (GeV/c$^2$)} \\
                                      & 200  & 300    & 400  & 500  & 500 \\\hline
Expected (MC)                         & 5.6 $\pm$ 1.5 & 5.2 $\pm$ 1.3 & 9.2 $\pm$ 2.0 & 18.5 $\pm$ 4.1 & 34.5 $\pm$ 7.5  \\
Data                                  & 5.1           & 4.7           & 8.6        & 17.1          & 31.2 \\\hline\hline
\end{tabular}
\end{center}
\end{table}
