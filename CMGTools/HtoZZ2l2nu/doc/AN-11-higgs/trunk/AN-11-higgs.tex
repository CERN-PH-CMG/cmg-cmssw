...
\RCS$Id: AN-11-higgs.tex,v 1.15 2011/09/11 12:34:00 psilva Exp $
%%%%%%%%%%%%% ptdr definitions %%%%%%%%%%%%%%%%%%%%%
...
Jets with $|\eta|>$2.5 are used for analysis but not for for jet counting. In the forward region due to the limited acceptance of the tracker
no jet-vertex association is possible, and therefore, the contamination from pileup becomes non negligible.
...
The ratio of the multiplicity of two consectuive jet bins (e.g. $R_k=N_k/N_{k+1}$ with $k$=1,2,...),  
...
For processes where jets are generated from colored decays or where jets accompany the fusion of heavy objects
...
to identify correctly the transition in the jet multiplicity reducing the contamination from pileup.
These arguments support our choice to count jets inside the tracker acceptance region only
(even if jets are used to higher $\eta$ regions). 
The VBF specific selection will be discussed in more detail in Sec.~\ref{sec:higgssearch}.

\begin{figure}[htp]
\begin{center}
%\includegraphics[width=0.4\textwidth]{img/njetsavgvspu}
%\includegraphics[width=0.4\textwidth]{img/rjetsvspu}
...
jets in the central region ($|\eta|<$2.5) and all reconstructed jets ($|\eta|<$5.0).}
