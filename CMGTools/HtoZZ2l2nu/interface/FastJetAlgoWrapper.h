...
// CODE MODIFIED ON JANUARAY 2012 IN ORDER TO ALLOW THE USE OF PFJET AS INPUT
...
namespace pf2pat {
  ...
  template <typename T, typename U>
    class FastJetAlgo {
	  ...
	  const JetCollection_U& produce( const std::vector< edm::Ptr< reco::Candidate > >  & inputColl, bool exclusive, 
          ...
          const JetCollection_U& produce( const std::vector< reco::PFJet >  & inputColl, bool exclusive,
                                          bool forced_ , double ptMin_, int n_jets); /////// overload to be able to rerun the algorithm on already produced jet 
	  ...
	  std::vector< edm::Ptr < reco::Candidate > > inputCollection_;  //// input vector needed to put all the containers in one vector 
          std::vector< reco::PFJet > inputJetCollection_;  //// input vector needed to put all the containers in one vector 
  ...
  //////// especialization for makeConstituents when using a vector of Jet as input   
  template<>
    reco::Jet::Constituents FastJetAlgo< std::vector< reco::PFJet > , reco::PFJet >::makeConstituents(const fastjet::PseudoJet& pseudoJet) const {
      reco::Jet::Constituents ptrsToConstituents;
  ...
  //// especialization of the produce function for a vector of PFJet as input and a vector of PFJet as output
  template<>
    const std::vector< reco::PFJet >& FastJetAlgo< std::vector< reco::PFJet >, reco::PFJet >::produce( const std::vector< reco::PFJet > & inputHandle, bool exclusive , bool forced_, double ptMin_, int n_jets) {
       // the input handle will be necessary to build the Ptrs to the jet constituents.
...
template<class T, class U>
void FastJetAlgo<T,U>::recoJetToFastJet(const std::vector< reco::PFJet >& inputColl) {
  input_.clear();
  unsigned i;
  for(i=0; i!=inputColl.size(); ++i) {
    input_.push_back( PseudoJet( inputColl[i].px(), inputColl[i].py(), inputColl[i].pz(), inputColl[i].energy() ) );
...
typedef pf2pat::FastJetAlgo< std::vector<reco::PFJet>, reco::PFJet > FastJetAlgoPFJetPFJet;
