  ...
  for ibin in bin:
    ...
    for mass in masses:
	  ...
	  for job in range(jobs):
            ...
            f_fit=file("Xvv.mX"+str(mass)+"_"+sChan[chan]+"_8TeV_channel"+str(ibin)+"_exp_toys"+str(job)+".out")
    ...
    sSigWW=[(masses[i],median(sigWW[i])) for i in range(len(masses))]
    sSigZZ=[(masses[i],median(sigZZ[i])) for i in range(len(masses))]

    sPvalWW=[(masses[i],median(pvalWW[i])) for i in range(len(masses))]
    sPvalZZ=[(masses[i],median(pvalZZ[i])) for i in range(len(masses))]

    fWW = open("Xvv_WW_8TeV_Pval_channel"+str(ibin)+"_exp_toys.txt", "w")
    fWW.write(str(sPvalWW))
    fWW.close()

    fZZ = open("Xvv_ZZ_8TeV_Pval_channel"+str(ibin)+"_exp_toys.txt", "w")
    fZZ.write(str(sPvalZZ))
    fZZ.close()

    fWW = open("Xvv_WW_8TeV_Sig_channel"+str(ibin)+"_exp_toys.txt", "w")
    fWW.write(str(sSigWW))
    fWW.close()

    fZZ = open("Xvv_ZZ_8TeV_Sig_channel"+str(ibin)+"_exp_toys.txt", "w")
