...

{
 ...
 for (int iSample = 0; iSample < 1; iSample++){
   ...
   string outFile("dijetHtag_RadionHH");
   ...
   for (int iMass = 0; iMass<massrange; iMass++){
     ...
     string sInFile = "~hinzmann/public/limit_input_files/" + inFile + "" + Form("OUT%d.root", 1000+iMass*100);
     ...
     for (int iCat = 0; iCat < 2; iCat++){
       ...
       TH1D* hMass = (TH1D*) file0.Get("DijetMassHighPuriHH;1");
       if (iCat == 1) hMass = (TH1D*) file0.Get("DijetMassHighPuriHH;1");
       if(!hMass) continue;
