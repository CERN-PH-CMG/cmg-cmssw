...
class PZetaRegions( Regions1D ):
    '''Checks event.diLepton.pZetaDisc()'''
    def __init__(self, cuts):
        super( PZetaRegions, self ).__init__()
        self.addRegion('LowPZeta', float('-inf'), cuts.pZeta_low)
        self.addRegion('HighPZeta', cuts.pZeta_high, float('inf') )
        
    def test(self, event):
        diLepton = event.diLepton
        return super( PZetaRegions, self ).test( diLepton.pZetaDisc() )
...
class H2TauTauRegions( object ):
    ...
    def __init__(self, cuts):
        ...
        self.dimensions.append( PZetaRegions(cuts) )
