...
from CMGTools.H2TauTau.proto.cuts_diTau import *
from CMGTools.H2TauTau.proto.xtitles import xtitles
from CMGTools.H2TauTau.proto.HistogramSet import histogramSet
from CMGTools.H2TauTau.proto.H2TauTauDataMC_diTau import *
from CMGTools.H2TauTau.proto.QCDEstimation_diTau import *
from CMGTools.H2TauTau.proto.DYEstimate_diTau import *
from CMGTools.H2TauTau.proto.SaveHistograms_diTau import *
from CMGTools.H2TauTau.proto.PrepareDictionaries_diTau import *
...
def printYields(region, plots, mass=-1, fileout=None, qcd=False) :
  ...
  if mass!= -1    : contributions.extend(['HiggsGGH' +str(mass),'HiggsVBF'+str(mass),'HiggsVH' +str(mass)])
...
if __name__ == '__main__':
  ...
  cuts=[  
      ("CMS_2012_ABCD_BOOSTED" , baseline + BOOSTED + NOVBF, '45', ' && pThiggs>140. ', isolationTM , 5 ), 
      ("CMS_2012_ABCD_VBF"     , baseline + BOOSTED + VBF  , '45', ' && pThiggs>110. ', isolationMM , 5 ),
       ]
  ...
  for prefix,cut,ptcut,antiqcdcut,isocut,qcdEstimate in cuts:
    ...
    #####################################################
    variables = hists_pref( rebin, mjjMin, mjjMax, mjjBin )
    ...
    for varLoop in variables:
      ...
      if yields:
        ...
        printYields('OS loose', LooseIsoOS, fileout=Yields_dump)       
        Yields_dump = open(os.getcwd()+"/"+prefix+"/Yields_SStight"+var+".txt","w")
        printYields('SS tight', TightIsoSS, fileout=Yields_dump)       
        Yields_dump = open(os.getcwd()+"/"+prefix+"/Yields_SSloose"+var+".txt","w")
        printYields('SS loose', LooseIsoSS, fileout=Yields_dump)            
      ...
      for mIndex in massesRange :    
        ...
        if yields :                                        
          ...
          printYields('OS tight', TightIsoOS, mass=mIndex, fileout=Yields_dump, qcd=True)            
        ...
        if datacard in var and isinstance(nx, numpy.ndarray) and 'VBF'     in prefix :
          saveForLimit(deepcopy(TightIsoOS),prefix,mIndex,datacard,"vbf"  )
        if datacard in var and isinstance(nx, numpy.ndarray) and 'BOOSTED' in prefix :
          saveForLimit(deepcopy(TightIsoOS),prefix,mIndex,datacard,"boost")
        ...
        TightIsoOS.Group('5x H('+str(mIndex)+' GeV)#rightarrow#tau#tau'  , ['HiggsVBF'+str(mIndex), 'HiggsGGH'+str(mIndex), 'HiggsVH'+str(mIndex)]) 
        ...
        if log: TightIsoOS.DrawStack("HIST",xmin,xmax,0.1,ymax)
        else  : TightIsoOS.DrawStack("HIST")
        ...
        if not datacard in var or ((len(nx) == 1 and nx<100000) or isinstance(nx, numpy.ndarray)) :
          saveForPlotting(deepcopy(TightIsoOS),prefix,mIndex)
