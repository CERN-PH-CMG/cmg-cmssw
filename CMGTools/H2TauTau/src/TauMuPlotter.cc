...
//--> problem with first bin  
  ...
  //Ratio for WJets //No correction for njet==2
  wjetsTauIsoRatio_ = "(1.0*(njet==0)+1.11*(njet==1)+1.11*(njet==2))*((20<taujetpt&&taujetpt<=25)*14.1+(25<taujetpt&&taujetpt<=30)*1.49+(30<taujetpt&&taujetpt<=35)*0.479+(35<taujetpt&&taujetpt<=40)*0.231+(40<taujetpt&&taujetpt<=45)*0.12+(45<taujetpt&&taujetpt<=50)*0.0825+(50<taujetpt&&taujetpt<=55)*0.0595+(55<taujetpt&&taujetpt<=60)*0.0435+(60<taujetpt&&taujetpt<=65)*0.0374+(65<taujetpt&&taujetpt<=70)*0.0319+(70<taujetpt&&taujetpt<=75)*0.0265+(75<taujetpt&&taujetpt<=80)*0.022+(80<taujetpt&&taujetpt<=85)*0.0177+(85<taujetpt&&taujetpt<=90)*0.0203+(90<taujetpt&&taujetpt<=95)*0.0139+(95<taujetpt&&taujetpt<=100)*0.0178)";
...
bool TauMuPlotter::correctSamplesInc(){
  ...
  if(WJetsOSSide>0&&WJetsSSSide>0){
...
TH1F* TauMuPlotter::getWJetsSM(){
  // samples with enough stats for the sideband correction
  if((SMcat_==-1 || SMcat_==0)
     || ((SMcat_==1||SMcat_==2) && Isocat_!=1)
     )
    return getWJetsInc();
  
  return getSample("WJetsToLNu");
}
TH1F* TauMuPlotter::getWJetsSMSS(){
  if((SMcat_==-1 || SMcat_==0)
     || ((SMcat_==1||SMcat_==2) && Isocat_!=1)
     )
    return getWJetsIncSS();
  
  return getSampleSS("WJetsToLNu");
}
...
TH1F* TauMuPlotter::getQCDIsoSM(){
  ...
  Float_t Berr=0.;
  for(Int_t b=1;b<=nbins_;b++) Berr+=hB->GetBinError(b)*hB->GetBinError(b);
  cout<<"QCD Region B "<<hDataB->Integral()<<" - "<<hMCB->Integral()<<"  ==> "<<hB->Integral()<<" +- "<<sqrt(Berr)<<endl;
  ...
  cout<<"QCD Region C "<<hDataC->Integral()<<" - "<<hMCC->Integral()<<"  ==> "<<hC->Integral()<<" +- "<<sqrt(Cerr)<<endl;
  ...
  Float_t Derr=0.;
  for(Int_t b=1;b<=nbins_;b++) Derr+=hD->GetBinError(b)*hD->GetBinError(b);
  cout<<"QCD Region D "<<hDataD->Integral()<<" - "<<hMCD->Integral()<<"  ==> "<<hD->Integral()<<" +- "<<sqrt(Derr)<<endl;
  ...
  Float_t QCDYield=(QCDYieldB/QCDerrB+QCDYieldC/QCDerrC+QCDYieldD/QCDerrD)/(1./QCDerrB+1./QCDerrC+1./QCDerrD);
  Float_t QCDerr=(1./sqrt(QCDerrB)+1./sqrt(QCDerrC)+1./sqrt(QCDerrD))/(1./QCDerrB+1./QCDerrC+1./QCDerrD);
  cout<<"QCDIso "<<QCDYield<<" +- "<<QCDerr<<endl;
...
TH1F* TauMuPlotter::getQCDIsoSMSS(){
  ...
  /////////////////////
  cout<<"Applying Tau Iso QCD ratio : "<<qcdTauIsoRatio_<<endl;
  extrasel_ += TString("*")+qcdTauIsoRatio_;//apply  ratio here 
  ...
  //Save the intermediate histograms in a root file
  TFile FQCDIsoSMSS(outputpath_+"/FQCDIsoSMSS_"+plotvar_+"_TauIso"+(long)Isocat_+"_MT"+(long)MTcat_+"_SM"+(long)SMcat_+".root","recreate");
  hData->Write();
  hMC->Write();
  h->Write();
  FQCDIsoSMSS.ls();
...
TH1F* TauMuPlotter::getWJetsTauIsoSM(){
  ...
  TH1F* h=new TH1F("hSMWJets","SMWJets",nbins_,xmin_,xmax_);
  h->Sumw2();
  ...
  TString tmpextrasel=extrasel_;
  extrasel_ += TString("*(nditau==1&&muiso<0.1)*")+wjetsTauIsoRatio_;//apply  ratio here 
  ...
  cout<<"WJetsTauIso : "<<h->Integral()<<endl;
  return h;

}


TH1F* TauMuPlotter::getWJetsTauIsoSMSS(){  //needed for cross-check

  TH1F* h=new TH1F("hSMWJetsSS","SMWJetsSS",nbins_,xmin_,xmax_);
  h->Sumw2();
  ...
  cout<<"Applying Tau Iso WJets ratio : "<<wjetsTauIsoRatio_<<endl;
  TString tmpextrasel=extrasel_;
  extrasel_ +=  TString("*(nditau==1&&muiso<0.1)*")+wjetsTauIsoRatio_;//apply pt dependent ratio here 

  ////////////
  TH1F* hW=getWJetsIncSS();
  h->Add(hW);
  delete hW;
  ////////////
  ...
  cout<<"WJetsTauIso SS : "<<h->Integral()<<endl;
  return h;
...
bool TauMuPlotter::plotSM(TString variable, Int_t Isocat, Int_t MTcat, Int_t SMcat, Int_t nbins, Float_t xmin, Float_t xmax, TString extrasel, TString xlabel, TString ylabel, Float_t* legendcoords, bool log){
  ...
  //TH1F*hWJetsToLNu=getWJetsTauIsoSM();
  if(!hWJetsToLNu)return 0;
  hWJetsToLNu->SetLineWidth(1);
  hWJetsToLNu->SetLineColor(WJetsColor_);
  hWJetsToLNu->SetFillColor(WJetsColor_);
...
void TauMuPlotter::plotTauFakeRateWJets(TString variable, Int_t nbins, Float_t xmin, Float_t xmax, TString extrasel, Float_t ymax,Bool_t log){
  ...
  MTcat_=3;
  SMcat_=-1;  
  extrasel_+="*(nditau==1&&muiso<0.1)";  //Ratio is computed only for IsoTaus / Anti-Iso taus , there is no gain from anti-iso muons
