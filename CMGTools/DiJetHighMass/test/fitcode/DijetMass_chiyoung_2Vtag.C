...
{
    double x=m[0]/7000.;
    return p[0]*pow(1.-x,p[1])/pow(x,p[2]+0*log(x));
}
/*
// QCD fit function -- alternate 2 Parameter fit function -- also used for QCD fit.
Double_t fitQCD2( Double_t *m, Double_t *p)
{
    double x=m[0]/7000.;
    return p[0]*pow(1.-x,p[1])/pow(x,0);
}
*/
// QCD fit function -- alternate 2 Parameter fit function -- also used for QCD fit.
Double_t fitQCD2( Double_t *m, Double_t *p)
{
    double x=m[0]/7000.;
    return p[0]*pow(1.-x,0)/pow(x,p[1]);
}


void DijetMass_chiyoung_2Vtag(){
...
//  lumi = 4677.0;
  ...
  TH1F *hPulls_2par = (TH1F*)hDijetMass->Clone("Pulls_2par");
...
//  pave->AddText("Coucou"); 
  ...
  */ 
  ...
  cout << "Fit QCD Up" << endl << endl << endl;
   ...
   // these are the graph format of data.
    ...
    TGraphAsymmErrors *g_2par = new TGraphAsymmErrors(i,vx,vy,vexl,vexh,veyl,veyh);
  ...
  TF1 *fit_lowMass = new TF1("fit_lowMass",fitQCD1,mMin,2050.0,4); // 4 Par. Fit
  gStyle->SetOptFit(1111); 
  ...
  g->Fit("fit_lowMass","","",mMin,2050.0);
    ...
    g->Fit("fit","","",mMin,2050.0);	
    TFitResultPtr fitResult=g->Fit("fit","S","",mMin,2050.0);	
    ...
    f_4par->SetParameter(3,6.49182e-05);

    f_4par->SetLineWidth(2);
    f_4par->SetLineColor(TColor::GetColor("#009900"));

    g_4par->Fit("fit_4par","","",mMin,2050.0);	
    g_4par->Fit("fit_4par","","",mMin,2050.0);
    g_4par->Fit("fit_4par","","",mMin,2050.0);
    ...
    f_2par->SetParameter(1,18.80678e+00);
    f_2par->SetLineWidth(2);
    f_2par->SetLineColor(2);
    f_2par->SetLineStyle(2);

    g_2par->Fit("fit_2par","","",mMin,2050.0);	
    g_2par->Fit("fit_2par","","",mMin,2050.0);	
    g_2par->Fit("fit_2par","","",mMin,2050.0);	
  ...
  TGraphAsymmErrors *gRatio_2par = new TGraphAsymmErrors(*gDefault);
  ...
  TGraphAsymmErrors *gPull_2par = new TGraphAsymmErrors(*gDefault);
  ...
  TMatrixDSym covarianceMatrix=fitResult->GetCovarianceMatrix();
	 ...
	 for(Int_t vi=1; vi<nPar;vi++){
	  ...
	  stringstream ss;
	  ss << "up" << vi;
	  TF1* variationUp=new TF1(ss.str().c_str(),fitQCD1,mMin,2050.0,4);
	  pars[0]=nbkgval;
	  pars[1]=p1val+gv(1);
	  pars[2]=p2val+gv(2);
	  pars[3]=p3val+gv(3);
	  ...
	  gv*= (*_Lt);
	  ss << "down";
	  TF1* variationDown=new TF1(ss.str().c_str(),fitQCD1,mMin,2050.0,4);
	  pars[0]=nbkgval;
	  pars[1]=p1val+gv(1);
	  pars[2]=p2val+gv(2);
	  pars[3]=p3val+gv(3);
  ...
  for (int iWindow = 0; iWindow < 15; iWindow++){
    ...
    TF1 *fit_window = new TF1(Form("fit_window_%d",iWindow),fitQCD1,mMin,2050.0,4); // 4 Par. Fit
    fit_window->SetParameter(0,1.73132e-04);
    fit_window->SetParameter(1,18.80678e+00);
    fit_window->SetParameter(2,3.33620e+00);
    ...
    gWindow->Fit(Form("fit_window_%d",iWindow),"","",mMin,2050.0);
    gWindow->Fit(Form("fit_window_%d",iWindow),"","",mMin,2050.0);
    ...
    for (int i = 0; i < gRatio->GetN(); i++){
      ...
      if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gRatio->SetPoint(i, fMass, XsecWindow/XsecDefault); 
      if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gRatio->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 

      if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gRatio->SetPoint(i, fMass, 1.0); 
      if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gRatio->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 
	...
	float XsecWindow_2par = f_2par->Eval(fMass,0,0);
	float XsecWindow_lowMass = fit_lowMass->Eval(fMass,0,0);

	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gRatio_4par->SetPoint(i, fMass, XsecWindow_4par/XsecDefault); 
	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gRatio_4par->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 

	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gRatio_2par->SetPoint(i, fMass, XsecWindow_2par/XsecDefault); 
	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gRatio_2par->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 

	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gRatio_lowMass->SetPoint(i, fMass, XsecWindow_lowMass/XsecDefault); 
	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gRatio_lowMass->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 


	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gRatio_4par->SetPoint(i, fMass, 1.0); 
	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gRatio_4par->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 

	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gRatio_2par->SetPoint(i, fMass, 1.0); 
	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gRatio_2par->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 

	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gRatio_lowMass->SetPoint(i, fMass, 1.0); 
	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gRatio_lowMass->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 
    ...
    for (int i = 0; i < gPull->GetN(); i++){
      ...
      if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gPull->SetPoint(i, fMass, (XsecWindow-XsecDefault)/XsecVariationDefault); 
      if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gPull->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 

      if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gPull->SetPoint(i, fMass, 0.0); 
      if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gPull->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 
	...
	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gPull_4par->SetPoint(i, fMass, (XsecWindow_4par-XsecDefault)/XsecVariationDefault); 
	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gPull_4par->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 

	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gPull_2par->SetPoint(i, fMass, (XsecWindow_2par-XsecDefault)/XsecVariationDefault); 
	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gPull_2par->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 

	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gPull_lowMass->SetPoint(i, fMass, (XsecWindow_lowMass-XsecDefault)/XsecVariationDefault); 
	if (Xsec > 1e-10 || (fMass < 2500.0 && fMass > mMin)) gPull_lowMass->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 


	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gPull_4par->SetPoint(i, fMass, 0.0); 
	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gPull_4par->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 

	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gPull_2par->SetPoint(i, fMass, 0.0); 
	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gPull_2par->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 

	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gPull_lowMass->SetPoint(i, fMass, 0.0); 
	if (Xsec < 1e-10 && (fMass > 2500.0 || fMass < mMin)) gPull_lowMass->SetPointError(i, 1e-10, 1e-10, 1e-10, 1e-10); 
 ...
 for (int iWindow = 0; iWindow < 15; iWindow++){ 
   ...
   ((TGraphAsymmErrors*) aData->At(iWindow))->GetXaxis()->SetRangeUser(890,2050.0);
...
c01->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithWindowFits.pdf");
    ...
    leg->SetLineWidth(1);
    leg->SetFillColor(0);
    leg->AddEntry(g,"Double W/Z-tag data","PL"); 
    leg->AddEntry(fit,"Fit","L");
    leg->Draw("same");
    pave_fit->Draw("same");
    c1->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithFit.png");
    c1->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithFit.pdf");
    ...
    //g2->Fit("fit_lowMass","+","sames",mMin,2050.0);
    ...
    cout << "*********************************************************"<<endl;
    double chi_2par = f_2par->GetChisquare();
    double ndf_2par = f_2par->GetNDF();
    cout << "Chisquare/ndf for 2 Par. Fit: " << chi_2par << "/" << ndf_2par << " : " << chi_2par/ndf_2par << endl;
    cout << "Status: "<<status_2par<<endl;
    cout << "*********************************************************"<<endl;
    //The end of Fit		
    ...
    htmp->SetLineColor(5);
    */
    c2->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithAllFit.png");
    c2->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithAllFit.pdf");
   ...
   c3->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithFitAndQCDMC.png");
   c3->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithFitAndQCDMC.pdf");
   ...
   c4->SaveAs("Plots_2Vtag/RatioDataPYTHIA.png");
   c4->SaveAs("Plots_2Vtag/RatioDataPYTHIA.pdf");
  ...
  // The end of comparison between data and PTYHIA QCD MC
   ...
   unsigned int qbin2 = 1;
   unsigned int sbin1 = 0;
   unsigned int sbin2 = 1;
   ...
   pt_c5_wprime1->AddText("G_{RS} #rightarrow ZZ (1 TeV)");
   ...
   pt_c5_wprime2->AddText("G_{RS} #rightarrow ZZ (1.5 TeV)");
   ...
   pt_c5_diquark1->AddText("G_{RS} #rightarrow WW (1 TeV)");
   ...
   pt_c5_diquark2->AddText("G_{RS} #rightarrow WW (1.5 TeV)");
   ...
   leg2->AddEntry(gr_diquark1,"G_{RS} #rightarrow WW","L");
   //leg2->AddEntry(gr_wprime1,"G_{RS} #rightarrow ZZ","L");
   leg2->Draw("same");
   pave_fit->Draw("same");

   double ratio[1000], ratio_lowMassFit[1000], pulls_2par[1000], pulls_4par[1000], pulls[1000], pulls_lowMassFit[1000], eyh_pulls_2par[1000], eyl_pulls_2par[1000], eyh_pulls_4par[1000], eyl_pulls_4par[1000], eyh_pulls[1000], eyl_pulls[1000], eyh_pulls_lowMassFit[1000], eyl_pulls_lowMassFit[1000];  
   ...
   for(int i=0;i<hDijetMass->GetNbinsX();i++)
	...
	double error = veyh[i];
	double m = vx[i];
	double fit_3p = f_2par->Eval(m,0,0);
	double fit_4p = f_4par->Eval(m,0,0);
	double fit_default = fit->Eval(m,0,0);  
	float fit_lowMassFit = fit_lowMass->Eval(m,0,0); 
	...
	if(data>=fit_lowMassFit) float error_lowMassFit = veyl[i];
	if (data<fit_lowMassFit) float error_lowMassFit = veyh[i];		

		
	pulls_2par[i]=(data-fit_3p)/fit_3p;
	eyh_pulls_2par[i] = veyh[i]/fit_3p;
	eyl_pulls_2par[i] = veyl[i]/fit_3p;
	...
	pulls_lowMassFit[i]=(data-fit_lowMassFit)/fit_lowMassFit;
	ratio_lowMassFit[i] = data / fit_lowMassFit;
	eyh_pulls_lowMassFit[i] = veyh[i]/fit_lowMassFit;
	...
	if(m<=565 || m>2050){
		...
		pulls_4par[i] = -999;
		pulls[i] = -999;
		pulls_lowMassFit[i] = -999;
	}
		
	if(error != 0.){
	  //	  std::cout << "m = " << m << std::endl;
	  double q_star=0.0;
		hPulls_2par->SetBinContent(i+1,(data-fit_3p)/error3p);
		hPulls_2par->SetBinError(i+1,1.);
   ...
   TGraphAsymmErrors *hDiff_2par = new TGraphAsymmErrors(i,vx,pulls_2par,vexl,vexh,eyl_pulls_2par,eyh_pulls_2par);  
   ...
   hPulls_2par->SetMarkerStyle(26);
   hPulls_2par->SetMarkerSize(1.5);
   hPulls_2par->SetMarkerColor(4);
   hPulls_2par->SetLineColor(4);
   hDiff_2par->SetMarkerStyle(26);
   hDiff_2par->SetMarkerSize(1.5);
   hDiff_2par->SetMarkerColor(4);
   hDiff_2par->SetLineColor(4);
   ...
   hPulls_lowMassFit->SetMarkerSize(1.5);
   hPulls_lowMassFit->SetMarkerColor(kMagenta);
   hPulls_lowMassFit->SetLineColor(kMagenta);
   ...
   c5->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithSignal.png");
   c5->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithSignal.pdf");
 ...
 //Dijet Mass Cross Section with Fit	
  ...
  ((TGraphAsymmErrors*) aRatio->At(0))->GetXaxis()->SetRangeUser(890,2050.0);
  ...
  gRatio_2par->SetLineWidth(3);
  gRatio_2par->SetLineStyle(1);
  gRatio_2par->SetLineColor(kBlue);
  ...
  legw->AddEntry(((TGraphAsymmErrors*) aRatio->At(0)),"15 fits with window/Default Fit (3 par.)","L");
  //legw->AddEntry(gRatio_lowMass,"Default Fit up to 1.4 TeV/Default Fit (3 par.)","L");
  legw->AddEntry(gRatio_4par,"Alternate Fit A (4 Par.)/Default Fit (3 par.)","L");
  legw->AddEntry(gRatio_2par,"Alternate Fit B (2 Par.)/Default Fit (3 par.)","L");
  legw->Draw("same");
  ...
  c02->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithWindowFitRatio.root");
...
/*
 ...
 //Dijet Mass Cross Section with Fit pulls
  ...
  ((TGraphAsymmErrors*) aPull->At(0))->GetXaxis()->SetRangeUser(890,2050.0);
  ...
  gPull_2par->SetLineWidth(3);
  gPull_2par->SetLineStyle(1);
  gPull_2par->SetLineColor(kBlue);
  ...
  legw2->AddEntry(gPull_2par,"Alternate Fit B (3 Par.) - Default Fit (4 par.)","L");
  legw2->Draw("same");



  c100->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithWindowFitPull.png");
  c100->SaveAs("Plots_2Vtag/DijetMassCrossSectionWithWindowFitPull.pdf");
...
*/
   ...
   pt_c6_diquark2->AddText("q* (1.5 TeV)");

   //pt_c6_diquark1->Draw("sames");
   //pt_c6_diquark2->Draw("sames");

   c6->SaveAs("Plots_2Vtag/DataMinusFitDividedByFit.png");
   c6->SaveAs("Plots_2Vtag/DataMinusFitDividedByFit.pdf");
   ...
   c7->SaveAs("Plots_2Vtag/DataMinusFitDividedByError.png");
   c7->SaveAs("Plots_2Vtag/DataMinusFitDividedByError.pdf");

   TCanvas* c8 = new TCanvas("c8","(Data-Fit)/Fit for the All Fits");
   hDiff->Draw("APZ");
   hDiff_4par->Draw("pzsame");
   hDiff_2par->Draw("pzsame");
   l->Draw("same"); 
   ...
   leg->Draw("same");
   c8->SaveAs("Plots_2Vtag/DataMinusFitDividedByFitForAllFits.png");
   c8->SaveAs("Plots_2Vtag/DataMinusFitDividedByFitForAllFits.pdf");


   TCanvas* c9 = new TCanvas("c9","(Data-Fit)/Error the All Fits");
   hPulls->Draw("ep");
   hPulls_4par->Draw("epsame");
   hPulls_2par->Draw("epsame");
   //hPulls_lowMassFit->Draw("epsame");
   ...
   c9->SaveAs("Plots_2Vtag/DataMinusFitDividedByErrorForAllFits.png");
   c9->SaveAs("Plots_2Vtag/DataMinusFitDividedByErrorForAllFits.pdf");	
   ...
   pt_c10_diquark2->AddText("q* (1.5 TeV)");
   ...
   c10->SaveAs("Plots_2Vtag/DataMinusFitWithAllSignal.png");
   c10->SaveAs("Plots_2Vtag/DataMinusFitWithAllSignal.pdf");
   ...
   //leg->AddEntry(htmp,"JES Uncertainty","F");
   leg->AddEntry(gr_diquark1,"G_{RS} #rightarrow WW","L");
   //leg->AddEntry(gr_wprime1,"G_{RS} #rightarrow ZZ","L");
   leg->Draw("same");
   ...
   pt_c11_wprime1->AddText("G_{RS} #rightarrow ZZ (1 TeV)");
   ...
   pt_c11_wprime2->AddText("G_{RS} #rightarrow ZZ (1.5 TeV)");
   ...
   pt_c11_diquark1->AddText("G_{RS} #rightarrow WW (1 TeV)");

   TPaveText *pt_c11_diquark2 = new TPaveText(0.25,0.27,0.60,0.35,"NDC");
   ...
   pt_c11_diquark2->AddText("G_{RS} #rightarrow WW (1.5 TeV)");
   ...
   TH1F *vFrame2 = p11_2->DrawFrame(890., -3, 2050.0, 3);
   vFrame2->SetTitle("");
