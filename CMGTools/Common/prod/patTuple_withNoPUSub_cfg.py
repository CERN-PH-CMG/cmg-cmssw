...
if runAK5LC:
    ...
    usePF2PAT(process,runPF2PAT=True, jetAlgo=jetAlgoAK5,
              runOnMC=runOnMC, postfix=postfixAK5LC,
              jetCorrections=('AK5PF', jetCorrections))

    getattr(process,"pfNoPileUp"+postfixAK5LC).enable = False
    getattr(process,"pfNoMuon"+postfixAK5LC).enable = False
    getattr(process,"pfNoElectron"+postfixAK5LC).enable = False
    getattr(process,"pfNoTau"+postfixAK5LC).enable = False
    
    addPATElectronID( process, 'patDefaultSequence', postfixAK5LC)
    print process.patDefaultSequenceAK5LC
    addMITElectronID( process, 'selectedPatElectrons', 'patDefaultSequence', postfixAK5LC)
