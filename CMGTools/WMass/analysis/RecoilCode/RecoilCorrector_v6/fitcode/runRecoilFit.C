...
/////
//Low ZPt analysis
TH1D histoZPt("hZPt","histo ZPt",100,0,100);
TH1D histodiffZPtGen("hdiffZPtGen","histo diffZPtGen",100,-5,5);
TH1D histodiffZPtOld("hdiffZPtOld","histo diffZPtOld",100,-5,5);
TH1D histoZMass("hZMass","histo Zmass",100,0,300);
TH2D histodiffZPtOldvsZPtGen("hdiffZPtOldvsZPtGen","histo diffZPtOld vs ZPtGen",100, 0, 50, 100,-5,5);
TH2D histodiffZPtGenvsZPtGen("hdiffZPtGenvsZPtGen","histo diffZPtGen vs ZPtGen",100, 0, 50, 100,-5,5);
TH2D histodiffZPtGenOldvsZPtGen("hdiffZPtGenOldvsZPtGen","histo diffZPtGenOld vs ZPtGen",100, 0, 50, 100,-5,5);
TH1D histodiffZPtGenOld("hdiffZPtGenOld","histo diffZPtGenOld",100,-5,5);

TH2D histoU1diffZpTgenvsZpt("hu1diffZpTgenvsZpt","histo (ZrecoPtparproj-Zpt) vs ZGenpt correction (-250MeV from PDG)",100,0,50,100,-50,50);
TH2D histoU1diffZpTgenvsZptOld("hu1diffZpTgenvsZptOld","histo (ZrecoPtparproj-Zpt) vs ZGenpt no correction",100,0,50,100,-50,50);

TH2D histoU1diffZpTgenvsZptU2("hu1diffZpTgenvsZptU2","histo (ZrecoPtperpproj-Zpt) vs ZGenpt correction (-250MeV from PDG)",100,0,50,100,-50,50);
TH2D histoU1diffZpTgenvsZptOldU2("hu1diffZpTgenvsZptOldU2","histo (ZrecoPtperpproj-Zpt) vs ZGenpt no correction",100,0,50,100,-50,50);


//TH2D histodistfZPtGen("hdistfZPtGen","histo distfZPtGen",100,0,5);
//TH2D histodistZPtGenOld("hdistZPtGenOld","histo distZPtGenOld",100,-5,5);
...
TH2D histoU1vsZpt("hu1vsZpt","histo U1 vs Zpt",100,0,50,100,-10,10);
TH2D histoU2vsZpt("hu2vsZpt","histo U2 vs Zpt",100,0,50,100,-50,50);
...
TH2D histoGenTkMetvsGenPfMet("histoGenTkMetvsGenPfMet","",100,0,50, 100,0,50);
TH2D histoGenTkMetvsTkMet("histoGenTkMetvsTkfMet","",100,0,50, 100,0,50);
...
TH1D histoNPV("hNPV","histo NPV",1000,0,50);
TH1D histoPhiStar("hPhiStar","histo PhiStar",1000,-10,10);
TH1D histoGenPhiStar("hGenPhiStar","histo GenPhiStar",1000,-10,10);
TH1D histoThetaStar("hThetaStar","histo ThetaStar",1000,-10,10);
TH2D histoPhiStarvsrecoZpt("histoPhiStarvsrecoZpt","histo PhiStar vs RecoZpt",1000,0,20,1000,0,0.5);
TH2D histoPhiStarvsZpt("histoPhiStarvsZpt","histo PhiStar vs Zpt",1000,0,20,1000,0,0.5);
TH2D histoU1vsPhiStar("histoU1vsPhiStar","histo U1 vs PhiStar",1000,-10,10,1000,0,0.5);
TH2D histodeltaPhivsZPtGen("histodeltaPhivsZPtGen","histo deltaPhi vs ZPtGen",100,0,50,100,-0.5,0.5);
...
bool doGenMet=false;
bool doBoost=false;
bool doTkMetABC=false;
bool doTkMetABC05=false;
bool doTkMet05=false;
bool doTkMetAB=false;
...
double fU1 = -99999; double fU2 = -99998;
...
double fZrecoPt = 0 , fZrecoPhi=0;

/*float fMet = 0;  float fMPhi = 0; */ 
double fMet = 0;  double fMPhi = 0;
double ftkMet = 0;  double ftkMPhi = 0; double fpfMet=0; double fgentkMet=0;
...
double fZmass, fZgenmass, fZ_mt;
double fMuPos_phi, fMuPos_eta, fMuPos_pt, fMuPos_charge, fMuPosReliso, fMuPos_dxy, fMuPos_dz, fMuPos_mass;
double fMuNeg_phi, fMuNeg_eta, fMuNeg_pt, fMuNeg_charge,fMuNeg_dxy, fMuNeg_dz, fMuNegReliso, fMuNeg_mass;
double fMuPosGen_eta, fMuPosGen_phi, fMuNegGen_eta, fMuNegGen_phi;
...
void load(TTree *iTree, int type) { 
  ...
  iTree->SetBranchAddress("MuNeg_pt" ,&fPt2); 
	...
	if((!doPosW) && (!doNegW)) {
		...
		if(!fData) iTree->SetBranchAddress("ZGen_mass" ,&fZgenmass);
		...
		iTree->SetBranchAddress("Z_mass" ,&fZmass); // used for runZselection
		iTree->SetBranchAddress("Z_mt" ,&fZ_mt);
		if(!fData) iTree->SetBranchAddress("Z_phi",&fZrecoPhi);
	...
	if(type==0) {
		...
		iTree->SetBranchAddress("tkmet_sumEt",&fSumEt);
		iTree->SetBranchAddress("tkmet_phi",&ftkMPhi);
		if(doGenMet){
			//iTree->SetBranchAddress("genpfmet",&fpfMet);
			iTree->SetBranchAddress("gentkmet",&ftkMet);
			iTree->SetBranchAddress("tkmet",&ftkMet);
			//iTree->SetBranchAddress("genpfmet_phi",&ftkMPhi);
		}

		if(do8TeV && doTkMetABC){
			iTree->SetBranchAddress("tkmetABC",&ftkMet);
			iTree->SetBranchAddress("tkmetABC_phi",&ftkMPhi);
			iTree->SetBranchAddress("tkmetABC_sumEt",&fSumEt);
		}
		if(do8TeV && doTkMetAB){
			iTree->SetBranchAddress("tkmetAB",&ftkMet);
			iTree->SetBranchAddress("tkmetAB_phi",&ftkMPhi);
			iTree->SetBranchAddress("tkmetAB_sumEt",&fSumEt);
		}
		if(do8TeV && doTkMetABC05){
			iTree->SetBranchAddress("tkmetABC050",&ftkMet);
			iTree->SetBranchAddress("tkmetABC050_phi",&ftkMPhi);
			iTree->SetBranchAddress("tkmetABC050_sumEt",&fSumEt);
		}
		if(do8TeV && doTkMet05){
			iTree->SetBranchAddress("tkmet050",&ftkMet);
			iTree->SetBranchAddress("tkmet050_phi",&ftkMPhi);
			iTree->SetBranchAddress("tkmet050_sumEt",&fSumEt);
		}
		if(donoPU){
	...
	if((!doPosW) && (!doNegW)) {
		...
		//GEN STUFF
		iTree->SetBranchAddress("MuPosGen_eta" ,&fMuPosGen_eta);
		iTree->SetBranchAddress("MuPosGen_phi" ,&fMuPosGen_phi);
		iTree->SetBranchAddress("MuNegGen_eta" ,&fMuNegGen_eta);
		iTree->SetBranchAddress("MuNegGen_phi" ,&fMuNegGen_phi);
...
void doBoostMET(){
	...
	TLorentzVector U(fMet,0,fMPhi,fSumEt);

	double energyZ=sqrt(fZgenmass*fZgenmass+fZPt*fZPt);
	TLorentzVector Z(fZPt, 0, fZPhi, energyZ);

	U.Boost( -Z.BoostVector() );
...
void calculateU1U2(double &iPar, bool iType) {
	...
	// cout << "fMet= " << fMet << " fMPhi= " << fMPhi << " fPt1= " << fPt1 << " fPhi1= " << fPhi1 << " fZPhi= " << fZPhi << " fU1= " << fU1 << endl;
	/*
    cout << "----------------" << endl;
...
double calculateSinThetaStar() {
	/*
	TLorentzVector MuPos;
	MuPos.SetPtEtaPhiM(fMuPos_pt, fMuPos_eta, fMuPos_phi, fMuPos_mass);

	TLorentzVector MuNeg;
	MuNeg.SetPtEtaPhiM(fMuNeg_pt, fMuNeg_eta, fMuNeg_phi, fMuNeg_mass);

	TLorentzVector Z;
	Z.SetPtEtaPhiM(fZPt, fZRap, fZPhi, fZmass);

	MuPos.Boost( -Z.BoostVector() );
	MuNeg.Boost( -Z.BoostVector() );
	 */
	return TMath::Sqrt(1-TMath::Power(TMath::TanH((fMuPos_eta-fMuNeg_eta)/2.),2));
}

double calculateCosThetaStar() {
	/*	TLorentzVector MuPos;
	MuPos.SetPtEtaPhiM(fMuPos_pt, fMuPos_eta, fMuPos_phi, fMuPos_mass);
	...
	MuNeg.Boost( -Z.BoostVector() );
	 */
	return TMath::TanH((fMuPos_eta-fMuNeg_eta)/2.);
}

double calculatePhiStar(double phi1, double phi2){

	float dphi = fabs( phi1 - phi2 );
	if( dphi > TMath::Pi() ) dphi = TMath::TwoPi() - dphi;
	double Phi_acop=TMath::Pi()-dphi;

	double SinThetaStar=calculateSinThetaStar();
...
double calculateDistance(double ZrecoPx, double ZrecoPy){

	return TMath::Sqrt( (fZPt*cos(fZPhi) - ZrecoPx)*(fZPt*cos(fZPhi) - ZrecoPx) + (fZPt*sin(fZPhi) - ZrecoPy)*(fZPt*sin(fZPhi) - ZrecoPy) );
...
bool runSelection() {
	...
	)
	{
		...
		/////////
		/// FILLING HISTOGRAMS
		// cout<<"1) prima del fill"<<endl;
		// cout<<"fU1"<<" "<<fU1<<" "<<"ZPt"<<" "<<fZPt<<" "<<"ZPhi"<<" "<<fZPhi<<" "<<"fPt1"<<" "<<fPt1<<" "<<"fPhi1"<<" "<<fPhi1<<" "<<endl;

		histodeltaPhivsZPtGen.Fill(fZPt, fZrecoPhi-fZPhi);
		histoU1vsZpt.Fill(fZPt,fU1);
		histoU2vsZpt.Fill(fZPt,fU2);
		...
		histoU2.Fill(fU2);
		histoGenTkMetvsGenPfMet.Fill(fpfMet,fgentkMet);
		histoGenTkMetvsTkMet.Fill(ftkMet,fgentkMet);
		histoNPV.Fill(fNPV);

		double SinThetaStar=calculateSinThetaStar();
		double CosThetaStar=calculateCosThetaStar();

		double PhiStar=calculatePhiStar(fMuPos_phi, fMuNeg_phi);
		double GenPhiStar=calculatePhiStar(fMuPosGen_phi, fMuNegGen_phi);
		histoPhiStarvsZpt.Fill(fZPt,fabs(PhiStar));
		histoPhiStar.Fill(PhiStar);
		histoThetaStar.Fill(CosThetaStar);
		histoU1vsPhiStar.Fill(fU1, fabs(PhiStar));
		histoGenPhiStar.Fill(GenPhiStar);
		histoPhiStarvsrecoZpt.Fill(fPt1, PhiStar);

		//Low ZPt analysis
		histoZPt.Fill(fZPt);
		histoZMass.Fill(fZmass);
		histodiffZPtGen.Fill(Z.Pt()-fZPt);
		histodiffZPtOld.Fill(Z.Pt()-fZrecoPt);
		histodiffZPtGenOld.Fill(fZPt-fZrecoPt);

		histodiffZPtGenvsZPtGen.Fill(fZPt, Z.Pt()-fZPt);
		histodiffZPtGenOldvsZPtGen.Fill(fZPt, fZrecoPt-fZPt);
		histodiffZPtOldvsZPtGen.Fill(fZPt, Z.Pt()-fZrecoPt);
		histoU1diffZpTgenvsZptOld.Fill(fZPt, fZrecoPt*cos(fZrecoPhi-fZPhi)-fZPt);
		histoU1diffZpTgenvsZpt.Fill(fZPt, Z.Pt()*cos(Z.Phi()-fZPhi)-fZPt);

		histoU1diffZpTgenvsZptOldU2.Fill(fZPt, fZrecoPt*sin(fZrecoPhi-fZPhi));
		histoU1diffZpTgenvsZptU2.Fill(fZPt, Z.Pt()*sin(Z.Phi()-fZPhi));

		//histodistfZPtGen.Fill(calculateDistance(Z.Px(), Z.Py()));
		//histodistZPtGenOld.Fill(calculateDistance(fZrecoPt*cos(fZPhi), fZrecoPt*sin(fZPhi)));
...
void computeFitErrors(TF1 *iFit,TFitResultPtr &iFPtr,TF1 *iTrueFit,bool iPol0=false) {
	...
	cout<<"iTrueFit   "<<iTrueFit->GetExpFormula()<<endl;
	cout<<"iFit   "<<iFit->GetExpFormula()<<endl;
	...
	iFit->SetParameter(5,iTrueFit->GetParameter(5)); iFit->SetParError(5,lE5);
	iFit->SetParameter(6,iTrueFit->GetParameter(6)); iFit->SetParError(6,lE6);
...
void fitGraph(TTree *iTree,TTree *iTree1, TCanvas *iC,
	...
	float binSize=1.;
	//float binSize=1;
	if(doPhiStar) binSize=1./100.;
	RooRealVar lRPt  ("pt","Z_{p_{T}}",range_min+binSize,range_max-binSize);
	...
	// this is the mean
	double minMean=-10.; double maxMean=10.;
	...
	if(doPhiStar && lPar==fU1) lRXVar.SetTitle("(U_{1}(#phi^{*})-x_{i})/#sigma_{U1} (#phi^{*})");
	if(doPhiStar && lPar!=fU1) lRXVar.SetTitle("(U_{2}(#phi^{*})-x_{i})/#sigma_{U2} (#phi^{*})");
	...
	/*
	if(fData && lPar==fU1) B1init = 1.5; // powheg U1
	if(fData && lPar==fU1) C1init = -2.0; // powheg U1
	if(fData && lPar==fU1) B2init = -3.0; // powheg U1
	if(fData && lPar==fU1) C2init = 13.0; // powheg U1
	 */
	...
	// ===> This is the model for the 2D (quadrati)
	...
	RooRealVar    lR1Mean ("Rmean1",  "Rmean1",0,minMean,maxMean);
	//RooRealVar    lR2Mean ("Rmean2",  "Rmean2",0,minMean,maxMean);
	//RooRealVar    lR2Mean ("Rmean",  "Rmean",0,minMean,maxMean); lR2Mean.setConstant(kTRUE);


	// ===> This is the model for the 1D (pallini)
	...
	TF1 *lFit = new TF1("test","pol3");
	//TF1 *lFit = new TF1("test","(TMath::Erf(x*[0])-TMath::Erf(x*[1]))*pol2(2)", range_min, range_max-binSize); //fZPtMax = 200;
	//if(lPar==fU1&&!iRMS) lFit->FixParameter(0,0);
	//if(lPar==fU1&&fData&&!iRMS) lFit = new TF1("test","TMath::Erf(x*[0])*pol2(1)");
	/*
	if(!iRMS && lPar==fU1 && !doPhiStar){
	...
	if(fData && lPar==fU1){
		lFit->SetParameter(0,-1.92915e-04);
		//f1->SetParLimits(0,-2, 2);
		//f1->SetParLimits(1,-2,2);
		lFit->SetParameter(1,-1.65518e+00);
		lFit->SetParameter(2,-1.91920e+00);
		//f1->SetParLimits(2,-2, 2);
		lFit->SetParameter(3, 4.38159e-02);
		lFit->SetParLimits(3,-1,1);
	...
	//	if(!iRMS && lPar==fU1) lFit->SetParameter(0,-2.0);
	//if(!iRMS && lPar==fU1) lFit->SetParLimits(0, 0.0, -2.0); // first parameter between 0 and -2.0
	...
	TFitResultPtr  lFitPtr = pGraphA->Fit(lFit,"SR","",range_min,range_max);
	TMatrixDSym lCov = lFitPtr->GetCovarianceMatrix();
	lCov.Print();
	//pGraphA->Fit(lFit, "R");
	...
	TF1 *lFit2 = new TF1("test","(TMath::Erf(x*[0])-TMath::Erf(x*[1]))*pol2(2)", range_min, range_max-binSize); //fZPtMax = 200;
	if(!fData && !iRMS && lPar==fU1) {
		lFit2->SetParameter(0,lFit->GetParameter(0));
		lFit2->SetParameter(1,lFit->GetParameter(1));
		//lFit2->FixParameter(1,1);
		...
		TFitResultPtr  lFitPtr2 = pGraphA->Fit(lFit2,"SRE","",range_min,range_max-binSize);
		computeFitErrors(iFit,lFitPtr2,lFit2,iRMS);
	} else if(fData && !iRMS && lPar==fU1) {
		lFit2 = new TF1("test","TMath::Erf(x*[0])*pol2(1)");
		lFit2->SetParameter(0,lFit->GetParameter(0));
		lFit2->SetParameter(1,lFit->GetParameter(1));
		//lFit2->FixParameter(1,1);
		//if(usePol3) lFit2->SetParameter(2,lFit->GetParameter(2));
	...
	else computeFitErrors(iFit,lFitPtr,lFit,iRMS);
	 */
	cout<<lPar<<"lPar"<<endl;
	cout<<fU1<<"fU1"<<endl;
	cout << "============"<< endl;
	...
	if(doPrint) {
		...
		if(doBoost) fileName2D +="boost";
		if(doGenMet) fileName2D +="GenMet";
		if(do8TeV) fileName2D+="_8TeV";
		if(doTkMetABC) fileName2D+="_TkMetABC";
		if(doTkMetAB) fileName2D+="_TkMetAB";
		if(doTkMetABC05) fileName2D+="_TkMetABC05";
		if(doTkMet05) fileName2D+="_TkMet05";
	...
	cout << "Eventi  " << lXVals_all.at(0).at(0).size() << endl;
	...
	for(unsigned int i0 = 0; i0 < lXVals_all.at(0).at(0).size(); i0++) {
		...
		//cout << "cardinality binned fit (1D)  " << " pId " << pId << endl;
	...
	TH1* hh1_ = lGaus1.createHistogram(nameHistoFitG1,lRPt,Binning(10000),YVar(lRXVar,Binning(100)));
	TH1* hh2_ = lGaus2.createHistogram(nameHistoFitG2,lRPt,Binning(10000),YVar(lRXVar,Binning(100)));
	...
	if(doPrintAll) {
		...
		for(int i0  = 0; i0 < lNBins; i0++) {
			...
			TString mean1str = Form("mean1 = %.2f",lR1Mean.getVal());
			latexLabel.DrawLatex(0.65, 0.6, mean1str);
			//TString mean2str = Form("mean2 = %.2f",lR2Mean.getVal());
			//latexLabel.DrawLatex(0.65, 0.55, mean2str);

			///////

			TString leg2 = "Zpt = ";
			if(doPhiStar) leg2 = "#phi^{*} = ";
			leg2 += i0*binSize;
			...
			if(doGenMet) test+="_GenMet";
			if(doBoost) test+="_Boost";
			if(do8TeV) test+="_8TeV";
			if(doTkMetABC) test+="_TkMetABC";
			if(doTkMetAB) test+="_TkMetAB";
			if(doTkMetABC05) test+="_TkMetABC05";
			if(doTkMet05) test+="_TkMet05";
			...
			lX[i0] = (range_max/lNBins)*i0;
			lEX[i0] = (range_max/lNBins)/10;
			// normalization too much
    ...
    if(lPar!=fU1) lG0->GetYaxis()->SetTitle("U2 #sigma_{mean}");
		...
		latexLabel.DrawLatex(0.25, 0.3, Form("lA1Sig = %f", lA1Sig.getVal()));
		latexLabel.DrawLatex(0.25, 0.35, Form("lB1Sig = %f", lB1Sig.getVal()));
		latexLabel.DrawLatex(0.25, 0.4, Form("lC1Sig = %f", lC1Sig.getVal()));
		latexLabel.DrawLatex(0.25, 0.45, Form("lA2Sig = %f", lA2Sig.getVal()));
		latexLabel.DrawLatex(0.25, 0.50, Form("lB2Sig = %f", lB2Sig.getVal()));
		latexLabel.DrawLatex(0.25, 0.55, Form("lC2Sig = %f", lC2Sig.getVal()));
		if(pFR) latexLabel.DrawLatex(0.25, 0.6, Form("status = %d", pFR->status()));


		/*
		TPaveLabel *t1 = new TPaveLabel(0.1,0.5,0.9,0.68, Form("lA1Sig = %f", lA1Sig.getVal()));
		TPaveLabel *t2 = new TPaveLabel(0.8,0.7,0.9,0.68, Form("lB1Sig = %f", lB1Sig.getVal()));
		TPaveLabel *t3 = new TPaveLabel(0.7,0.6,0.9,0.68, Form("lC1Sig = %f", lC1Sig.getVal()));
		TPaveLabel *t4 = new TPaveLabel(0.7,0.6,0.9,0.68, Form("lA2Sig = %f", lA2Sig.getVal()));
		TPaveLabel *t5 = new TPaveLabel(0.7,0.6,0.9,0.68, Form("lB2Sig = %f", lB2Sig.getVal()));
		TPaveLabel *t6 = new TPaveLabel(0.7,0.6,0.9,0.68, Form("lC2Sig = %f", lC2Sig.getVal()));
		...
		if(doGenMet) pSS0+="_GenMet";
		if(doBoost) pSS0+="_Boost";
		if(do8TeV) pSS0+="_8TeV";
		if(doTkMetABC) pSS0+="_TkMetABC";
		if(doTkMetAB) pSS0+="_TkMetAB";
		if(doTkMetABC05) pSS0+="_TkMetABC05";
		if(doTkMet05) pSS0+="_TkMet05";
    ...
    iC->SaveAs(pSS0.str().c_str());
		...
		TF1 *lFitPullMean  = new TF1("lFitPullMean",   "pol3");
		TF1 *iFitPullMean  = new TF1("iFitPullMean",   "pol10"); // this need to be large to store the errors from the computeFitErrors

		TFitResultPtr  lFitPtrM0 = lM0->Fit(lFitPullMean,"SR","",range_min+binSize , range_max-binSize); //"EXO"
		...
		if(doGenMet) pMM0+="_GenMet";
		if(doBoost) pMM0+="_Boost";
		if(do8TeV) pMM0+="_8TeV";
		if(doTkMetABC) pMM0+="_TkMetABC";
		if(doTkMetAB) pMM0+="_TkMetAB";
		if(doTkMetABC05) pMM0+="_TkMetABC05";
		if(doTkMet05) pMM0+="_TkMet05";
		...
		TFitResultPtr  lFitPtr1 = lG1->Fit(lFitS1,"SRE","", range_min+binSize, range_max-binSize); //"EXO"
		computeFitErrors(iFitS1,lFitPtr1,lFitS1,iRMS);

		lG1->GetYaxis()->SetRangeUser(minSigma1-0.2,maxSigma1+0.2);
		...
		latexLabel.DrawLatex(0.25, 0.35, Form("lA1Sig = %f", lA1Sig.getVal()));
		latexLabel.DrawLatex(0.25, 0.4, Form("lB1Sig = %f", lB1Sig.getVal()));
		latexLabel.DrawLatex(0.25, 0.45, Form("lC1Sig = %f", lC1Sig.getVal()));
		latexLabel.DrawLatex(0.25, 0.5, Form("lA2Sig = %f", lA2Sig.getVal()));
		latexLabel.DrawLatex(0.25, 0.55, Form("lB2Sig = %f", lB2Sig.getVal()));
		latexLabel.DrawLatex(0.25, 0.6, Form("lC2Sig = %f", lC2Sig.getVal()));
		if(pFR) latexLabel.DrawLatex(0.25, 0.65, Form("status = %d", pFR->status()));
		...
		if(doGenMet) pSS1+="_GenMet";
		if(doBoost) pSS1+="_Boost";
		if(do8TeV) pSS1+="_8TeV";
		if(doTkMetABC) pSS1+="_TkMetABC";
		if(doTkMetAB) pSS1+="_TkMetAB";
		if(doTkMetABC05) pSS1+="_TkMetABC05";
		if(doTkMet05) pSS1+="_TkMet05";
    ...
    iC->SaveAs(pG1.str().c_str());
		...
		TFitResultPtr  lFitPtr2 = lG2->Fit(lFitS2,"SRE","", range_min+binSize, range_max-binSize);
		computeFitErrors(iFitS2,lFitPtr2,lFitS2,iRMS);

		//lG2->GetYaxis()->SetRangeUser(minSigma2-0.2,maxSigma2+0.2);
		...
		if(doGenMet) pSS2+="_GenMet";
		if(doBoost) pSS2+="_Boost";
		if(do8TeV) pSS2+="_8TeV";
		if(doTkMetABC) pSS2+="_TkMetABC";
		if(doTkMetAB) pSS2+="_TkMetAB";
		if(doTkMetABC05) pSS2+="_TkMetABC05";
		if(doTkMet05) pSS2+="_TkMet05";
    ...
    // cin.get();
		...
		if(doGenMet) pSS4+="_GenMet";
		if(doBoost) pSS4+="_Boost";
		if(do8TeV) pSS4+="_8TeV";
		if(doTkMetABC) pSS4+="_TkMetABC";
		if(doTkMetAB) pSS4+="_TkMetAB";
		if(doTkMetABC05) pSS4+="_TkMetABC05";
		if(doTkMet05) pSS4+="_TkMet05";
 ...
 */
void loopOverTree(TTree *iTree, bool isBKG=false, int whatToDo=1, std::string iName="") {
	...
	TString iname = iName.c_str();
	iname.ReplaceAll(".root","");
	iname.ReplaceAll("recoilfit_","");

	TObjArray* tokenized = iname.Tokenize("/");
	iname=((TObjString *)tokenized->At(tokenized->GetEntriesFast()-1))->GetString();
	...
	if(abs(whatToDo)==1){
		...
		TFile *fout_loopOverTree=new TFile(Form("fout_loopOverTree_%s.root",iname.Data()),"RECREATE");
		TTree *selEvTree= new TTree("selEvTree","selEvTree");
		TBranch* br_lXVals_all = selEvTree->Branch("lXVals_all", &(lXVals_all[0][0]));
		TBranch* br_lXEVals_all = selEvTree->Branch("lXEVals_all", &(lXEVals_all[0][0]));
		TBranch* br_lYVals_U1 = selEvTree->Branch("lYVals_U1", &(lYVals_all[0][0]));
		TBranch* br_lYVals_U2 = selEvTree->Branch("lYVals_U2", &(lYVals_all[1][0]));
		TBranch* br_lYEVals_all = selEvTree->Branch("lYEVals_all", &(lYEVals_all[0][0]));
      ...
      if(!fData && !(fEvent % 2)) continue;
			...
			// this is Z selection
			double pippo1=0;
			// cout<<"1) prima di runSelection"<<endl;
			//cout<<lPar<<"lPar prima di calc"<<endl;
			// cout<<fU1<<"fU1 prima di calc"<<endl;
			calculateU1U2(pippo1,true);
			//cout<<lPar<<"lPar dopo calc"<<endl;
			// cout<<fU1<<"fU1 dopo calc"<<endl;
			if((!doPosW) && (!doNegW) && (!runSelection()))  continue;
			// cout<<"2) dopo runSelection"<<endl;
			// calculateU1U2(pippo1,true);
       ...
       << endl;
				...
				if(!doApplyCorr) {
					...
					float dphi = fabs(fMuPos_phi - fMuNeg_phi);
					if( dphi > TMath::Pi() ) dphi = TMath::TwoPi() - dphi;
					double PhiStar=calculatePhiStar(fMuPos_phi, fMuNeg_phi);

					applyType2CorrMET(fMet,fMPhi,
							fZPt,fZPhi, // gen for parametriation
							fPt1,fPhi1, // to add to the MEt to get the u
							lZMU1Fit[0],
    ...
    fMPhi = newPFMET.Phi();
      ...
      if(iPol1 && fZPt > 50. && lPar == fU1 && lPar  > (47.5-0.95*fZPt) && iType == 0) continue; //Remove Diboson events at high pT
		...
		TFile *fin_loopOverTree=new TFile(Form("fout_loopOverTree_%s.root",iname.Data()));
		fin_loopOverTree->Print();
		TTree *selEvTree= (TTree*)fin_loopOverTree->Get("selEvTree");
		TBranch *br_lXVals_all,*br_lXEVals_all,*br_lYVals_U1,*br_lYVals_U2,*br_lYEVals_all;
		std::vector<float> *b_lXVals_all = new std::vector<float>,
				*b_lXEVals_all = new std::vector<float>,
				*b_lYVals_U1 = new std::vector<float>,
				*b_lYVals_U2 = new std::vector<float>,
				*b_lYEVals_all = new std::vector<float>;

		selEvTree->SetMakeClass(1);
		selEvTree->SetBranchAddress("lXVals_all", &b_lXVals_all, &br_lXVals_all);
		selEvTree->SetBranchAddress("lXEVals_all", &b_lXEVals_all, &br_lXEVals_all);
		selEvTree->SetBranchAddress("lYVals_U1", &b_lYVals_U1, &br_lYVals_U1);
		selEvTree->SetBranchAddress("lYVals_U2", &b_lYVals_U2, &br_lYVals_U2);
		selEvTree->SetBranchAddress("lYEVals_all", &b_lYEVals_all, &br_lYEVals_all);

		bool U1U2truefalse[2]={true,false};

		selEvTree->GetEntry(0);
		for(int isel=0; isel<b_lXVals_all->size(); isel++){
			if(isel%100000==0) cout <<"Retrieved selected entry "<< isel <<"/"<< selEvTree->GetEntries() <<endl;
			for(int iU1U2=0; iU1U2<2; iU1U2++){

				for(int iMeanRMS=0; iMeanRMS<2; iMeanRMS++){

					bool isU1=U1U2truefalse[iU1U2];
					lXVals_all.at(iU1U2).at(iMeanRMS).push_back(b_lXVals_all->at(isel));
					lXEVals_all.at(iU1U2).at(iMeanRMS).push_back(b_lXEVals_all->at(isel));

					double pWeight = fWeight;
					if(fData) pWeight = 1;
					lYVals_all.at(iU1U2).at(iMeanRMS).push_back(isU1?b_lYVals_U1->at(isel):b_lYVals_U2->at(isel));
					lYTVals_all.at(iU1U2).at(iMeanRMS).push_back(isU1?b_lYVals_U1->at(isel):b_lYVals_U2->at(isel));
					lYEVals_all.at(iU1U2).at(iMeanRMS).push_back(b_lYEVals_all->at(isel));
...
void fitRecoilMET(TTree *iTree,std::string iName,int type, int lfId, int whatToDo) {
	...
	loopOverTree(iTree,false,whatToDo,iName);
	//  fU2=fU2*(-1);
	...
	//TF1 *lU1Fit     = new TF1((lPrefix+"u1Mean_"+PUstring.str()).c_str(),   "TMath::Erf(x*[0])*pol2(1)");
...
void fitRecoilMETMIX(TTree *iTree, TTree *iTreeBKG, std::string iName,int type, int lfId, int whatToDo) {
	std::string lPrefix="PF"; if(type == 1) lPrefix = "TK";
	load(iTree,type);
	loopOverTree(iTree,false,whatToDo,iName);

	load(iTreeBKG,type);
	loopOverTree(iTreeBKG,true,whatToDo,iName);
...
void fitRecoil(TTree *iTree,std::string iName,int ifId,int whatToDo) {
	fitRecoilMET(iTree,iName,2,ifId,whatToDo);
	//  fitRecoilMET(iTree,iName,0);
	//  fitRecoilMET(iTree,iName,1);
	//  makeCorr(iTree,iName);
}

void fitRecoilMIX(TTree *iTree,TTree *iTreeBKG,std::string iName,int ifId, int whatToDo) {
	fitRecoilMETMIX(iTree,iTreeBKG,iName,2,ifId,whatToDo);
	//  fitRecoilMET(iTree,iName,0);
	//  fitRecoilMET(iTree,iName,1);
	//  makeCorr(iTree,iName);
}
...
void runRecoilFit(int MCtype, int iloop, int processType, int whatToDo) {
	...
	TString name="recoilfits/recoilfit_AUG21";
	if(do8TeV) name +="_8TeV";
	if(fNPV<15) name +="_le15VTX";
	if(fNPV>15) name +="_gt15VTX";
	if(doTkMetABC) name +="_TkMetABC";
	if(doTkMetAB) name +="_TkMetAB";
	if(doTkMetABC05) name +="_TkMetABC05";
	if(doTkMet05) name +="_TkMet05";
	if(doBoost) name+="_Boost";
	...
	if(MCtype==1) {
		...
		if(doIterativeMet) {
			...
			if(!doMad) readRecoil(lZMSumEt,lZMU1Fit,lZMU1RMSSMFit,lZMU1RMS1Fit,lZMU1RMS2Fit,/*lZMU13SigFit,*/lZMU2Fit,lZMU2RMSSMFit,lZMU2RMS1Fit,lZMU2RMS2Fit,/*lZMU23SigFit,*/"/afs/cern.ch/work/e/emanca/public/CMSSW_5_3_19/src/CMGTools/WMass/analysis/RecoilCode/RecoilCorrector_v6/fitcode/recoilfits/recoilfit_AUG21_le15VTX_genZ_tkmet_eta21_MZ81101_PDF-1_pol3_type2_doubleGauss_x2Stat_UNBINNED_53X_powheg.root" ,"PF",fId);
			if(doMad) readRecoil(lZMSumEt,lZMU1Fit,lZMU1RMSSMFit,lZMU1RMS1Fit,lZMU1RMS2Fit,/*lZMU13SigFit,*/lZMU2Fit,lZMU2RMSSMFit,lZMU2RMS1Fit,lZMU2RMS2Fit,/*lZMU23SigFit,*/"/afs/cern.ch/work/e/emanca/public/CMSSW_5_3_19/src/CMGTools/WMass/analysis/RecoilCode/RecoilCorrector_v6/fitcode/recoilfits/recoilfit_AUG21_le15VTX_genZ_tkmet_eta21_MZ81101_PDF-1_pol3_type2_doubleGauss_x2Stat_UNBINNED_53X_madgraph.root" ,"PF",fId);
			readRecoil(lZDSumEt,lZDU1Fit,lZDU1RMSSMFit,lZDU1RMS1Fit,lZDU1RMS2Fit,/*lZDU13SigFit,*/lZDU2Fit,lZDU2RMSSMFit,lZDU2RMS1Fit,lZDU2RMS2Fit,/*lZDU23SigFit,*/"/afs/cern.ch/work/e/emanca/public/CMSSW_5_3_19/src/CMGTools/WMass/analysis/RecoilCode/RecoilCorrector_v6/fitcode/recoilfits/recoilfit_AUG21_le15VTX_DATA_tkmet_eta21_MZ81101_pol3_type2_doubleGauss_x2Stat_UNBINNED_53X.root" ,"PF",fId);
	...
	if(MCtype==3) {
		...
		if(doMad&&(!do8TeV)) fDataFile = TFile::Open("root://eoscms//eos/cms/store/group/phys_smp/Wmass/perrozzi/ntuples/ntuples_2014_08_19_53X_gentkmet/WJetsLL/WTreeProducer_tree_SignalRecoSkimmed.root");
		if(!doMad) fDataFile = TFile::Open("root://eoscms//eos/cms/store/group/phys_smp/Wmass/perrozzi/ntuples/ntuples_2014_08_19_53X_gentkmet/WPlusPOWHEG/WTreeProducer_tree.root");
	...
	if(MCtype==4) {
		...
		//    fDataFile = TFile::Open("root://eoscms//eos/cms/store/group/phys_smp/Wmass/perrozzi/ntuples/ntuples_2014_03_04/WJetsLL/WTreeProducer_tree.root");
		if(doMad&&(!do8TeV)) fDataFile = TFile::Open("root://eoscms//eos/cms/store/group/phys_smp/Wmass/perrozzi/ntuples/ntuples_2014_08_19_53X_gentkmet/WJetsLL/WTreeProducer_tree_SignalRecoSkimmed.root");
		if(!doMad) fDataFile = TFile::Open("root://eoscms//eos/cms/store/group/phys_smp/Wmass/perrozzi/ntuples/ntuples_2014_08_19_53X_gentkmet/WMinusPOWHEG/WTreeProducer_tree.root");
	...
	if(MCtype==50) {
		fitRecoilMIX(fDataTree,fTTbarTree,name.Data(), fId,whatToDo);
	} else {
		fitRecoil(fDataTree, name.Data(), fId, whatToDo);
	}
	...
	if(doPrint) {
		...
		if(do8TeV) fileName +="8TeV";
		if(doGenMet) fileName +="GenTkMET";
		...
		histoU1vsPhiStar.Write();
		histoGenPhiStar.Write();
		histoPhiStarvsrecoZpt.Write();
		histoGenTkMetvsGenPfMet.Write();
		histoGenTkMetvsTkMet.Write();
		histoNPV.Write();

		//Low ZPt analysis
		histoZPt.Write();
		histoZMass.Write();
		histodiffZPtGen.Write();
		histodiffZPtOld.Write();
		histodiffZPtOldvsZPtGen.Write();
		histodiffZPtGenvsZPtGen.Write();
		histodiffZPtGenOld.Write();
		histodiffZPtGenOldvsZPtGen.Write();
		histoU1diffZpTgenvsZptOld.Write();
		histoU1diffZpTgenvsZpt.Write();
		histoU1diffZpTgenvsZptOldU2.Write();
		histoU1diffZpTgenvsZptU2.Write();
		//histodistZPtGenOld.Write();
		//histodistfZPtGen.Write();
		f3.Write();
		f3.Close();
