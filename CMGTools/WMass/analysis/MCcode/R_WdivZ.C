...
void R_WdivZ(TString folder="", int OnlyCounts=0, int tot_N_evts=1){
  ...
  TFile *Wana=new TFile(OnlyCounts ? Form("%s/WanalysisOnDATA.root",folder.Data()) : Form("%s/WanalysisOnMC.root",folder.Data()));
  TFile *Zana=new TFile(OnlyCounts ? Form("%s/ZanalysisOnDATA.root",folder.Data()) : Form("%s/ZanalysisOnMC.root",folder.Data()));
    ...
    for(int j=0; j<2*WMass::WMassNSteps+1; j++){
      ...
      for(int kcut=0; kcut<CutSteps; kcut++){
        ...
        if(!OnlyCounts){
          ...
          hR_WdivZ_WlikePos[kcut][i][j].SetLineColor(line_marker_color[kcut]);
          hR_WdivZ_WlikePos[kcut][i][j].SetMarkerColor(line_marker_color[kcut]);
          hR_WdivZ_WlikePos[kcut][i][j].SetLineStyle(linestyle[kcut]);
          hR_WdivZ_WlikePos[kcut][i][j].SetLineWidth(2);
          hR_WdivZ_WlikePos[kcut][i][j].SetMarkerStyle(20);
          hR_WdivZ_WlikePos[kcut][i][j].SetMarkerSize(0.7);
          hR_WdivZ_WlikePos[kcut][i][j].GetXaxis()->SetRangeUser(0.7,1.6);
          hR_WdivZ_WlikePos[kcut][i][j].GetYaxis()->SetRangeUser(0.6,1.5);
          hR_WdivZ_WlikePos[kcut][i][j].DrawCopy(kcut==0?"histo":Form("%s sames",draw_options[kcut].Data()));
          TString legend_name = hR_WdivZana_str[kcut];
          legend_name.ReplaceAll("hR_WdivZ_WlikePos_","");
          TLegendEntry *entry=leg->AddEntry((TH1D*)&hR_WdivZ_WlikePos[kcut][i][j],legend_name,"p");
  ...
  if(!OnlyCounts){
    ...
    for(int i=WMass::etaMuonNSteps-1; i>=0; i--){
      ...
      for(int kcut=4; kcut<CutSteps; kcut++){
          ...
          // cout << Form("mass %d eta %s",jWmass,eta_str.Data()) << endl;
          TH1D*hWPos_PtScaled=(TH1D*)Wana->Get(Form("%s_eta%s_%d",Wana_str[kcut].Data(),eta_str.Data(),jWmass));
          cout << Form("%s ",Wana_str[kcut].Data()) << " Eff= " << (hWPos_PtScaled->GetEntries()/tot_N_evts*2) << endl;
        ...
        for(int kcut=4; kcut<CutSteps; kcut++){
          ...
          // cout << Form("mass %d eta %s",jWmass,eta_str.Data()) << endl;
          TH1D*hWlikePos_PtScaled=(TH1D*)Zana->Get(Form("%s_eta%s_%d",Zana_str[kcut].Data(),eta_str.Data(),jWmass));
          cout << Form("%s ",Zana_str[kcut].Data()) << " Eff= " << (hWlikePos_PtScaled->GetEntries()/tot_N_evts*2) << endl;
  ...
  // TH1D hWtemplate[WMass::etaMuonNSteps][2*WMass::WMassNSteps+1];
