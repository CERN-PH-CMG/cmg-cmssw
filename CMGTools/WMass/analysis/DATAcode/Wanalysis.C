...
void Wanalysis::Loop(int IS_MC_CLOSURE_TEST, int isMCorDATA, TString outputdir, int useMomentumCorr, int smearRochCorrByNsigma, int useEffSF, int useVtxSF, TString sampleName)
{
  ...
  TH1D *hpfMET_WPos[WMass::nSigOrQCD][WMass::etaMuonNSteps], *hpfMETphi_WPos[WMass::nSigOrQCD][WMass::etaMuonNSteps], *hWPos_pt[WMass::nSigOrQCD][WMass::etaMuonNSteps];
  TH1D *hWPos_phi[WMass::nSigOrQCD][WMass::etaMuonNSteps], *hWPos_mt[WMass::nSigOrQCD][WMass::etaMuonNSteps], *hMupt_WPos[WMass::nSigOrQCD][WMass::etaMuonNSteps];
  TH1D *hMueta_WPos[WMass::nSigOrQCD][WMass::etaMuonNSteps], *hMuphi_WPos[WMass::nSigOrQCD][WMass::etaMuonNSteps], *hMulogiso_WPos[WMass::nSigOrQCD][WMass::etaMuonNSteps], *hJetpt_WPos[WMass::nSigOrQCD][WMass::etaMuonNSteps];
  TH1D *hJeteta_WPos[WMass::nSigOrQCD][WMass::etaMuonNSteps], *hJetphi_WPos[WMass::nSigOrQCD][WMass::etaMuonNSteps], *hWPos_MuDRgen[WMass::nSigOrQCD][WMass::etaMuonNSteps];
  TH2D *hWPos_logiso_vs_logdxy[WMass::etaMuonNSteps];
  TH2D *hWPos_iso_vs_dxy[WMass::etaMuonNSteps];
  ...
  for(int i=0; i<WMass::etaMuonNSteps; i++){
    ...
    hWPos_logiso_vs_logdxy[i]=new TH2D(Form("hWPos_logiso_vs_logdxy_eta%s",eta_str.Data()),Form("hWPos_logiso_vs_logdxy_eta%s",eta_str.Data()),1000,-5,1,1000,-5,1);
    hWPos_iso_vs_dxy[i]=new TH2D(Form("hWPos_iso_vs_dxy_eta%s",eta_str.Data()),Form("hWPos_iso_vs_dxy_eta%s",eta_str.Data()),1000,-0.001,0.999,1000,0,1);
    
    for(int k=0; k<WMass::nSigOrQCD; k++){
      hnvtx[k][i]=new TH1D(Form("hnvtx_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hnvtx_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),200,0,200);
      hpfMET_WPos[k][i]=new TH1D(Form("hpfMET_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hpfMET_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),200,0,200);
      hpfMETphi_WPos[k][i]=new TH1D(Form("hpfMETphi_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hpfMETphi_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),100,-TMath::Pi(),TMath::Pi());
      hWPos_pt[k][i]=new TH1D(Form("hWPos_pt_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hWPos_pt_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),100,0,25);
      hWPos_phi[k][i]=new TH1D(Form("hWPos_phi_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hWPos_phi_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),100,-TMath::Pi(),TMath::Pi());
      hWPos_mt[k][i]=new TH1D(Form("hWPos_mt_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hWPos_mt_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),200,0,200);
      hMupt_WPos[k][i]=new TH1D(Form("hMupt_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hMupt_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),200,0,200);
      hMueta_WPos[k][i]=new TH1D(Form("hMueta_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hMueta_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),100,-2.5,2.5);
      hMuphi_WPos[k][i]=new TH1D(Form("hMuphi_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hMuphi_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),100,-TMath::Pi(),TMath::Pi());
      hMulogiso_WPos[k][i]=new TH1D(Form("hMulogiso_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hMulogiso_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),1000,-5,3);
      hJetpt_WPos[k][i]=new TH1D(Form("hJetpt_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hJetpt_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),100,0,50);
      hJeteta_WPos[k][i]=new TH1D(Form("hJeteta_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hJeteta_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),100,-2.5,2.5);
      hJetphi_WPos[k][i]=new TH1D(Form("hJetphi_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hJetphi_WPos_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),100,-TMath::Pi(),TMath::Pi());
      hWPos_MuDRgen[k][i]=new TH1D(Form("hWPos_MuDRgen_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),Form("hWPos_MuDRgen_%s_eta%s",WMass::nSigOrQCD_str[k].Data(),eta_str.Data()),1000,-6,1);
    }
    for(int j=0; j<2*WMass::WMassNSteps+1; j++){
      ...
      hWPos_PtScaled_QCD[i][j]=new TH1D(Form("hWPos_PtScaled_QCD_eta%s_%d",eta_str.Data(),jWmass),Form("hWPos_PtScaled_QCD_eta%s_%d",eta_str.Data(),jWmass),nbins,bins);
  ...
  rochcor44X *rmcor = new rochcor44X();  // make the pointer of rochcor class
  ...
  cout << "using card " << fitParametersFile << endl;
  MuScleFitCorrector *corrector = new MuScleFitCorrector(fitParametersFile);
  
  Long64_t nbytes = 0, nb = 0;
  for (Long64_t jentry=first_entry; jentry<nentries;jentry++) {
  // for (Long64_t jentry=0; jentry<5e3;jentry++) {
    ...
    if(useEffSF && (IS_MC_CLOSURE_TEST || isMCorDATA==0)){
      MuPos_tight_muon_SF = hEffSF_MuId_eta_2011[runopt]->Eval(Mu_eta)*hEffSF_Iso_eta_2011[runopt]->Eval(Mu_eta);
    }
          ...
          if(evtHasGoodVtx && evtHasTrg && Mu_charge>0){
            ...
            if(useMomentumCorr){
              if(IS_MC_CLOSURE_TEST || isMCorDATA==0){
                ...
                rmcor->momcor_mc(mu, Mu_charge, smearRochCorrByNsigma/* , runopt */);
              }
              else{
                rmcor->momcor_data(mu, Mu_charge, smearRochCorrByNsigma/* , run<175832 ? 0 : 1  */);
              }
            ...
            if( /* Z_mass>50  not possible to apply something similar to Z mass cut*/
              ...
              if(MuIsTightAndIso && MuRelIso<0.12 && Mu_dxy<0.02){
                ...
                if(pfmet>25){
                  ...
                  if(W_pt<20){
                    ...
                    // if(Jet_leading_pt<30){
                      ...
                      if(useGenVar) hWPos_MuDRgen[0][i]->Fill(TMath::Log10(MuDRGenP),evt_weight*MuPos_tight_muon_SF);
                      hnvtx[0][i]->Fill(nvtx,evt_weight); // TO FIT MET IN THE WHOLE RANGE!!!!
                      hpfMETphi_WPos[0][i]->Fill(pfmet_phi,evt_weight*MuPos_tight_muon_SF);
                      hWPos_pt[0][i]->Fill(W_pt,evt_weight*MuPos_tight_muon_SF);
                      hWPos_phi[0][i]->Fill(W_phi,evt_weight*MuPos_tight_muon_SF);
                      hWPos_mt[0][i]->Fill(W_mt,evt_weight*MuPos_tight_muon_SF);
                      hMupt_WPos[0][i]->Fill(mu.Pt(),evt_weight*MuPos_tight_muon_SF);
                      hMueta_WPos[0][i]->Fill(mu.Eta(),evt_weight*MuPos_tight_muon_SF);
                      hMuphi_WPos[0][i]->Fill(mu.Phi(),evt_weight*MuPos_tight_muon_SF);
                      hMulogiso_WPos[0][i]->Fill(TMath::Log10(MuRelIso),evt_weight*MuPos_tight_muon_SF);
                      hJetpt_WPos[0][i]->Fill(Jet_leading_pt,evt_weight*MuPos_tight_muon_SF);
                      hJeteta_WPos[0][i]->Fill(Jet_leading_eta,evt_weight*MuPos_tight_muon_SF);
                      hJetphi_WPos[0][i]->Fill(Jet_leading_phi,evt_weight*MuPos_tight_muon_SF);
                ...
                if(W_pt<20){ // do not cut on MET to let the fit have an handle
                  ...
                  hpfMET_WPos[0][i]->Fill(pfmet,evt_weight*MuPos_tight_muon_SF); // TO FIT MET IN THE WHOLE RANGE!!!!
                ...
                if(pfmet>25 && W_pt<20){
                  if( (TMath::Abs(wmass1 - WMass::WMassCentral_MeV) > 1)) continue;
                  hWPos_logiso_vs_logdxy[i]->Fill(TMath::Log10(Mu_dxy),TMath::Log10(MuRelIso),evt_weight*MuPos_tight_muon_SF);
                  hWPos_iso_vs_dxy[i]->Fill(Mu_dxy,MuRelIso,evt_weight*MuPos_tight_muon_SF);
                }
                ...
                if(
                  ...
                  hpfMET_WPos[1][i]->Fill(pfmet,evt_weight*MuPos_tight_muon_SF);  // TO FIT MET IN THE WHOLE RANGE!!!!  
                  ...
                  if(pfmet>25){
                    ...
                    if(useGenVar) hWPos_MuDRgen[1][i]->Fill(TMath::Log10(MuDRGenP),evt_weight*MuPos_tight_muon_SF);
                    hnvtx[1][i]->Fill(nvtx,evt_weight);  // TO FIT MET IN THE WHOLE RANGE!!!!  
                    hpfMETphi_WPos[1][i]->Fill(pfmet_phi,evt_weight*MuPos_tight_muon_SF);
                    hWPos_pt[1][i]->Fill(W_pt,evt_weight*MuPos_tight_muon_SF);
                    hWPos_phi[1][i]->Fill(W_phi,evt_weight*MuPos_tight_muon_SF);
                    hWPos_mt[1][i]->Fill(W_mt,evt_weight*MuPos_tight_muon_SF);
                    hMupt_WPos[1][i]->Fill(mu.Pt(),evt_weight*MuPos_tight_muon_SF);
                    hMueta_WPos[1][i]->Fill(mu.Eta(),evt_weight*MuPos_tight_muon_SF);
                    hMuphi_WPos[1][i]->Fill(mu.Phi(),evt_weight*MuPos_tight_muon_SF);
                    hMulogiso_WPos[1][i]->Fill(TMath::Log10(MuRelIso),evt_weight*MuPos_tight_muon_SF);
                    hJetpt_WPos[1][i]->Fill(Jet_leading_pt,evt_weight*MuPos_tight_muon_SF);
                    hJeteta_WPos[1][i]->Fill(Jet_leading_eta,evt_weight*MuPos_tight_muon_SF);
                    hJetphi_WPos[1][i]->Fill(Jet_leading_phi,evt_weight*MuPos_tight_muon_SF);
                  }
                }
              }
  ...
  for(int i=0; i<WMass::etaMuonNSteps; i++){
    ...
    for(int k=0; k<WMass::nSigOrQCD; k++){
      hWPos_MuDRgen[k][i]->Write();
      hnvtx[k][i]->Write();
      hpfMET_WPos[k][i]->Write();
      hpfMETphi_WPos[k][i]->Write();
      hWPos_pt[k][i]->Write();
      hWPos_phi[k][i]->Write();
      hWPos_mt[k][i]->Write();
      hMupt_WPos[k][i]->Write();
      hMueta_WPos[k][i]->Write();
      hMuphi_WPos[k][i]->Write();
      hMulogiso_WPos[k][i]->Write();
      hJetpt_WPos[k][i]->Write();
      hJeteta_WPos[k][i]->Write();
      hJetphi_WPos[k][i]->Write();
    }
    for(int j=0; j<2*WMass::WMassNSteps+1; j++){
