...
void Wanalysis::Loop(int IS_MC_CLOSURE_TEST, int isMCorDATA, TString outputdir, int useMomentumCorr, int smearRochCorrByNsigma, int useEffSF, int usePtSF, int useVtxSF, int controlplots, TString sampleName, int generated_PDF_set, int generated_PDF_member, int contains_PDF_reweight)
{
  ...
  // for (Long64_t jentry=0; jentry<1e5;jentry++) { // for testing purposes
    ...
    if((IS_MC_CLOSURE_TEST || isMCorDATA==0) && controlplots) common_stuff::plot1D("hPileUp_Fall11",
                                    npu, 1, h_1d, 50,0,50
                                  );
          ...
          if(WGen_m>0){ // only for signal event
            if(MuGen_charge>0){ // only for positive muons
              ...
              for(int k=0;k<3;k++){
                ...
                common_stuff::plot1D(Form("hWPos_%sScaled_1_Gen_eta%s_%d",WMass::FitVar_str[k].Data(),eta_str.Data(),jWmass),
                ...
                common_stuff::plot1D(Form("hWPos_%sScaled_2_ZGenMassCut_eta%s_%d",WMass::FitVar_str[k].Data(),eta_str.Data(),jWmass),
                                    MuPosGen_var_jacobian[k], evt_weight, h_1d, nbins, bins_scaled[k]
                                  );
              }
                ...
                if(TMath::Abs(MuGen_eta)<WMass::etaMaxMuons[i]){
                  for(int k=0;k<3;k++){
                    ...
                    common_stuff::plot1D(Form("hWPos_%sScaled_3_Mu1GenCut_eta%s_%d",WMass::FitVar_str[k].Data(),eta_str.Data(),jWmass),
                                    MuPosGen_var_jacobian[k], evt_weight, h_1d, nbins, bins_scaled[k]
                                  );
                    ...
                    // if(TMath::Abs(MuNegGen_eta)<2.4){
                      ...
                      common_stuff::plot1D(Form("hWPos_%sScaled_4_Mu2GenCut_eta%s_%d",WMass::FitVar_str[k].Data(),eta_str.Data(),jWmass),
                                    MuPosGen_var_jacobian[k], evt_weight, h_1d, nbins, bins_scaled[k]
                                  );
                    // }
                  }
        ...
        if(!useGenVar || W_mt>0){ // dummy thing to separate between sig and bkg in W+Jets (useless)
          for(int m=0; m<WMass::NtoysMomCorr; m++){
            ...
            if(evtHasGoodVtx && evtHasTrg && Mu_charge>0){
              ...
              if(dphiGen < -TMath::Pi()) dphiGen += 2*TMath::Pi();
              double MuPos_var_NotScaledGen[3] = {MuGen_pt,WGen_mt,dphiGen}; // SCALED VARIABLE
              ...
              if( /* Z_mass>50  not possible to apply something similar to Z mass cut*/
                ...
                if(MuIsTightAndIso && MuRelIso<0.12 && Mu_dxy<0.02 && noTrgMuonsLeadingPt<10 ){
                  for(int k=0;k<3;k++)
                    ...
                    if(m==0) common_stuff::plot1D(Form("hWPos_%sScaled_5_RecoCut_eta%s_%d",WMass::FitVar_str[k].Data(),eta_str.Data(),jWmass),
                  ...
                  if(pfmet>25){
                    for(int k=0;k<3;k++)
                      ...
                      if(m==0) common_stuff::plot1D(Form("hWPos_%sScaled_6_METCut_eta%s_%d",WMass::FitVar_str[k].Data(),eta_str.Data(),jWmass),
                                    MuPos_var_jacobian[k],evt_weight*MuPos_tight_muon_SF, h_1d, nbins, bins_scaled[k]
                                  );
                    ...
                    if(W_pt<20){
                      for(int k=0;k<3;k++)
                        ...
                        if(m==0) common_stuff::plot1D(Form("hWPos_%sScaled_7_RecoilCut_eta%s_%d",WMass::FitVar_str[k].Data(),eta_str.Data(),jWmass),
                                    MuPos_var_jacobian[k],evt_weight*MuPos_tight_muon_SF, h_1d, nbins,bins_scaled[k]
                                  );
                      ...
                      // if(Jet_leading_pt<30){
                        ...
                        for(int h=0; h<WMass::PDF_members; h++){
                          ...
                          for(int k=0;k<3;k++){
                            ...
                            common_stuff::plot1D(Form("hWPos_%sScaled_8_JetCut_pdf%d-%d%s_eta%s_%d",WMass::FitVar_str[k].Data(),WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,toys_str.Data(),eta_str.Data(),jWmass),
                                    MuPos_var_jacobian[k],evt_weight*MuPos_tight_muon_SF*lha_weight, h_1d, nbins, bins_scaled[k]
                                  );
                            ...
                            // hWPos_VarNonScaled_8_JetCut[m][h][k][i][j]->Fill(MuPos_var_NotScaled[k]*iWmass/(WMass::WMassCentral_MeV/1e3)<xmax*80/(k==1 ? 1 : 2) ? MuPos_var_NotScaledGen[k] : (xmax-binsize2/2)*80/(k==1 ? 1 : 2) ,evt_weight*MuPos_tight_muon_SF*lha_weight);
                            
                            common_stuff::plot1D(Form("hWPos_%sNonScaled_8_JetCut_pdf%d-%d%s_eta%s_%d",WMass::FitVar_str[k].Data(),WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,toys_str.Data(),eta_str.Data(),jWmass),
                                    MuPos_var_NotScaled[k]*iWmass/(WMass::WMassCentral_MeV/1e3)<xmax*80/(k==1 ? 1 : 2) ? MuPos_var_NotScaled[k] : (xmax-binsize2/2)*80/(k==1 ? 1 : 2) ,evt_weight*MuPos_tight_muon_SF*lha_weight, 
                                    h_1d, nbins, bins_Notscaled[k]
                                  );
                          }
                        }
                        ...
                        if(controlplots){
                          ...
                          if(useGenVar) common_stuff::plot1D(Form("hWPos_MuDRgen_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    TMath::Log10(MuDRGenP),evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 1000,-6,1
                                  );
                          ...
                          common_stuff::plot1D(Form("hnoTrgMuonsLeadingPt_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    noTrgMuonsLeadingPt, evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 300, -100, 200
                                  );
                          // hpfMETphi_WPos[0][i][j]->Fill(pfmet_phi,evt_weight*MuPos_tight_muon_SF);
                          common_stuff::plot1D(Form("hpfMETphi_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                          ...
                          common_stuff::plot1D(Form("hWPos_pt_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    W_pt, evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100, 0, 25
                                  );
                          // hWPos_phi[0][i][j]->Fill(W_phi,evt_weight*MuPos_tight_muon_SF);
                          common_stuff::plot1D(Form("hWPos_phi_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    W_phi, evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,-TMath::Pi(),TMath::Pi()
                                  );
                          // hWPos_mt[0][i][j]->Fill(W_mt,evt_weight*MuPos_tight_muon_SF);
                          common_stuff::plot1D(Form("hWPos_mt_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    W_mt, evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 200,0,200
                                  );
                          // hMupt_WPos[0][i][j]->Fill(mu.Pt(),evt_weight*MuPos_tight_muon_SF);
                          common_stuff::plot1D(Form("hMupt_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    mu.Pt(),evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 200,0,200
                                  );
                          // hMueta_WPos[0][i][j]->Fill(mu.Eta(),evt_weight*MuPos_tight_muon_SF);
                          common_stuff::plot1D(Form("hMueta_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    mu.Eta(),evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,-2.5,2.5
                                  );
                          // hMuphi_WPos[0][i][j]->Fill(mu.Phi(),evt_weight*MuPos_tight_muon_SF);
                          common_stuff::plot1D(Form("hMuphi_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    mu.Phi(),evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,-TMath::Pi(),TMath::Pi()
                                  );
                          // hMulogiso_WPos[0][i][j]->Fill(TMath::Log10(MuRelIso),evt_weight*MuPos_tight_muon_SF);
                          common_stuff::plot1D(Form("hMulogiso_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    TMath::Log10(MuRelIso),evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 1000,-5,3
                                  );
                          // hJetpt_WPos[0][i][j]->Fill(Jet_leading_pt,evt_weight*MuPos_tight_muon_SF);
                          common_stuff::plot1D(Form("hJetpt_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    Jet_leading_pt,evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,0,50
                                  );
                          // hJeteta_WPos[0][i][j]->Fill(Jet_leading_eta,evt_weight*MuPos_tight_muon_SF);
                          common_stuff::plot1D(Form("hJeteta_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                                    Jet_leading_eta,evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,-2.5,2.5
                                  );
                          // hJetphi_WPos[0][i][j]->Fill(Jet_leading_phi,evt_weight*MuPos_tight_muon_SF);
                          common_stuff::plot1D(Form("hJetphi_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data()),
                  ...
                  if(controlplots && W_pt<20 && m==0){ // do not cut on MET to let the fit have an handle
                    ...
                    common_stuff::plot1D(Form("hpfMET_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[0].Data(),eta_str.Data(),jWmass),
                  ...
                  if(
                    ...
                    common_stuff::plot1D(Form("hpfMET_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    pfmet,evt_weight*MuPos_tight_muon_SF, 
                                    h_1d, 200,0,200
                                  );
                    ...
                    if(pfmet>25){
                      for(int k=0;k<3;k++)
                        ...
                        common_stuff::plot1D(Form("hWPos_%sScaled_QCD_eta%s_%d",WMass::FitVar_str[k].Data(),eta_str.Data(),jWmass),
                                    MuPos_var_jacobian[k],evt_weight*MuPos_tight_muon_SF, h_1d, nbins, bins_scaled[k]
                                  );
                      ...
                      common_stuff::plot1D(Form("hWPos_MuDRgen_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    TMath::Log10(MuDRGenP),evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 1000,-6,1
                                  );
                      ...
                      common_stuff::plot1D(Form("hpfMETphi_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    pfmet_phi,evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100, -TMath::Pi(), TMath::Pi()
                                  );
                      // hWPos_pt[1][i][j]->Fill(W_pt,evt_weight*MuPos_tight_muon_SF);
                      common_stuff::plot1D(Form("hWPos_pt_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    W_pt, evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100, 0, 25
                                  );
                      // hWPos_phi[1][i][j]->Fill(W_phi,evt_weight*MuPos_tight_muon_SF);
                      common_stuff::plot1D(Form("hWPos_phi_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    W_phi, evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,-TMath::Pi(),TMath::Pi()
                                  );
                      // hWPos_mt[1][i][j]->Fill(W_mt,evt_weight*MuPos_tight_muon_SF);
                      common_stuff::plot1D(Form("hWPos_mt_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    W_mt, evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 200,0,200
                                  );
                      // hMupt_WPos[1][i][j]->Fill(mu.Pt(),evt_weight*MuPos_tight_muon_SF);
                      common_stuff::plot1D(Form("hMupt_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    mu.Pt(),evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 200,0,200
                                  );
                      // hMueta_WPos[1][i][j]->Fill(mu.Eta(),evt_weight*MuPos_tight_muon_SF);
                      common_stuff::plot1D(Form("hMueta_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    mu.Eta(),evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,-2.5,2.5
                                  );
                      // hMuphi_WPos[1][i][j]->Fill(mu.Phi(),evt_weight*MuPos_tight_muon_SF);
                      common_stuff::plot1D(Form("hMuphi_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    mu.Phi(),evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,-TMath::Pi(),TMath::Pi()
                                  );
                      // hMulogiso_WPos[1][i][j]->Fill(TMath::Log10(MuRelIso),evt_weight*MuPos_tight_muon_SF);
                      common_stuff::plot1D(Form("hMulogiso_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    TMath::Log10(MuRelIso),evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 1000,-5,3
                                  );
                      // hJetpt_WPos[1][i][j]->Fill(Jet_leading_pt,evt_weight*MuPos_tight_muon_SF);
                      common_stuff::plot1D(Form("hJetpt_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    Jet_leading_pt,evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,0,50
                                  );
                      // hJeteta_WPos[1][i][j]->Fill(Jet_leading_eta,evt_weight*MuPos_tight_muon_SF);
                      common_stuff::plot1D(Form("hJeteta_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data(),jWmass),
                                    Jet_leading_eta,evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,-2.5,2.5
                                  );
                      // hJetphi_WPos[1][i][j]->Fill(Jet_leading_phi,evt_weight*MuPos_tight_muon_SF);
                      common_stuff::plot1D(Form("hJetphi_WPos_%s_eta%s_%d",WMass::nSigOrQCD_str[1].Data(),eta_str.Data()),
                                    Jet_leading_phi,evt_weight*MuPos_tight_muon_SF,
                                    h_1d, 100,-TMath::Pi(),TMath::Pi()
                                  );
                    }
