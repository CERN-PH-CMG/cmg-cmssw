...
double offset_from_string(string str, double limit)
{
  ...
  for (int i=0; i<len; ++i) {
    ...
    accumulator = fmod(accumulator, limit);
  }
...
void merge_results(int generated_PDF_set=1, int generated_PDF_member=0, TString WorZ="W", int useBatch=0, int RecoilCorrVarDiagoParU1orU2fromDATAorMC=0, string offset_string=""){
  ...
  for(unsigned int itoken=0; itoken<tokenized.size(); itoken++){
    ...
    massCentral_MeV -= offset_from_string(offset_string, 1000);
          ...
          for(int n=0; n<WMass::KalmanNvariations; n++){
            ...
            for(int i=0; i<WMass::etaMuonNSteps; i++){
              ...
              for(int k=0;k<WMass::NFitVar;k++){
                ...
                ffit[i][k][c]=new TF1(Form("ffit_W%s%s_%s_pdf%d-%d%s%s_eta%s",Wlike.Data(),WCharge_str[c].Data(),WMass::FitVar_str[k].Data(),WMass::PDF_sets<0?generated_PDF_set:WMass::PDF_sets,h,RecoilCorrVarDiagoParU1orU2fromDATAorMC>0?Form("_RecoilCorrVar%d",m):"",WMass::KalmanNvariations>1?Form("_KalmanVar%d",n):"",eta_str.Data()),Form("[0]+TMath::Power((x-[1]-%f)/[2],2)",offset_from_string(offset_string,1000)),70,100);
                ffit[i][k][c]->SetParameter(0,result_NonScaled[i][m-m_start][h][k][c]->GetMinimum());
                ffit[i][k][c]->SetParameter(1,WorZ.Contains("W")?80410:91170 + offset_from_string(offset_string,1000));
