...
void TemplateFit(TString fWDATA_str=0, TString fMCtemplates_str=0, TString fR_str=0, int fitType=0/* , double xmin=0.75, double xmax=1.4  */){
  ...
  TFile*fWtemplatesFromZDATA=new TFile(Form("%s",fMCtemplates_str.Data()));
  TFile*fR=new TFile(Form("%s",fR_str.Data()));

  TFile *fout=new TFile("FitResults.root","RECREATE");
  
  TH1D *hWDATA;                              // hWDATA histogram
  TH1D *hWtemplateFromZDATA;                               // MC histogram
  ...
  for(int fitlimits=0; fitlimits<FitCombinations; fitlimits++){
    ...
    for(int i=0; i<WMass::etaMuonNSteps; i++){
      ...
      // for(int j=0; j<1; j++){
        ...
        hWDATA = (TH1D*)fWDATA->Get(Form("hWPos_PtScaled_8_JetCut_eta%s_%d",eta_str.Data(),jWmass));
        hWtemplateFromZDATA = (TH1D*)fWtemplatesFromZDATA->Get(Form("hWlikePos_PtScaled_RWeighted_Templates_eta%s_%d",eta_str.Data(),jWmass));
        hR = (TH1D*)fR->Get(Form("hR_WdivZ_WlikePos_8_JetCut_eta%s_%d",eta_str.Data(),jWmass));

        hWDATA->GetXaxis()->SetRangeUser(lowerFit[fitlimits],upperFit[fitlimits]);
        hWtemplateFromZDATA->GetXaxis()->SetRangeUser(lowerFit[fitlimits],upperFit[fitlimits]);
        
        chi2_fitres= fitType==0 ? hWDATA->Chi2Test(hWtemplateFromZDATA,"UU NORM CHI2/NDF") : CustomChi2Test(hWDATA,hWtemplateFromZDATA,hR,lowerFit[fitlimits],upperFit[fitlimits]);
        // chi2_fitres= fitType==0 ? hWDATA->Chi2Test(hWtemplateFromZDATA,"WW CHI2/NDF") : CustomChi2Test(hWDATA,hWtemplateFromZDATA,hR,lowerFit[fitlimits],upperFit[fitlimits]);
        // chi2_fitres=CustomChi2Test(hWDATA,hWtemplateFromZDATA,hR,xmin,xmax);
...
double CustomChi2Test(TH1D *Wdata, TH1D *mcZtemplates, TH1D *hR, double xmin, double xmax){
  ...
  for(int i=xmin_bin; i<xmax_bin; i++){
    double WdataPoint,ZdatascaledPoint,sigmaWdata,sigmaZdatascaled;
    WdataPoint = Wdata->GetBinContent(i)*ZdivW_norm;
    ZdatascaledPoint = mcZtemplates->GetBinContent(i);
    sigmaZdatascaled = TMath::Sqrt(mcZtemplates->GetBinContent(i)*hR->GetBinContent(i));
    // sigmaWdata = TMath::Sqrt(Wdata->GetBinContent(i)*ZdivW_norm);
    sigmaWdata = 0;

    chi2+= TMath::Power( ( WdataPoint - ZdatascaledPoint ) / TMath::Hypot(sigmaWdata,sigmaZdatascaled) , 2 );
    // cout << WdataPoint << " " << ZdatascaledPoint << " " << sigmaWdata << " " << sigmaZdatascaled << " " << hR->GetBinContent(i) << " " << chi2 << endl;
  }
