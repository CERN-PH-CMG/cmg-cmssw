{
  ...
  TF2 *fpol = new TF2("fpol","[0]*0.5*(1-3*x*x)+[1]*sin(2*acos(x))*cos(y)+[2]*0.5*sin(acos(x))*sin(acos(x))*cos(2*y)+[3]*sin(acos(x))*cos(y)+[4]*x + [5]*(1+x*x)",-1,1,0,TMath::Pi());
  ...
  for(int irap=0; irap<nrapbins; irap++){
    ...
    TCanvas *cai[fitcoeff];
    for(int i=0; i<fitcoeff; i++){
      cai[i] = new TCanvas(Form("ca%d_rap%d",i,irap),Form("ca%d_rap%d",i,irap));
      cai[i]->SetLogx();
      haibins_rap[irap][i]->GetXaxis()->SetRangeUser(1,haibins_rap[irap][i]->GetXaxis()->GetXmax());
      ...
      fout->cd();
      cai[i]->Write();
      cai[i]->SaveAs(Form("ca%d_rap%d.pdf",i,irap));
    }
  }
