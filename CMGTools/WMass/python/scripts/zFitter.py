...
def printCanvas(c1, name, text, options, xoffs=0, spam=[]):
    ...
    doSpam(lumitext+" (8 TeV)", .55+xoffs, .955, .97, .998, align=32, textSize=options.textSize)
...
def makeHist1D(tree, options, weightedMC = False):
    ...
    if weightedMC: finalCut = "(%s) * puWeight " % finalCut
...
def addZFitterOptions(parser):
    ...
    parser.add_option("-t", "--tree",    dest="tree", default='treeProducerWMassEle', help="Tree name");
    parser.add_option("-c", "--cut",     dest="cut", type="string", default="mZ1 > 0", help="cut")
    parser.add_option("--xcut",     dest="xcut", type="float", nargs=2, default=None, help="x axis cut")
    parser.add_option("-x", "--x-var",   dest="xvar", type="string", default=("mZ1","80,70,110"), nargs=2, help="X var and bin")
    ...
    parser.add_option("-l","--lumi",   dest="lumi", type="float", default=19.7, help="Text size")
