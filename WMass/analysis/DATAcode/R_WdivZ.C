...
void R_WdivZ(TString folder="", int OnlyCounts=0, int tot_N_evts=1){
  ...
  TFile *Wana=new TFile(Form("%s/WanalysisOnDATA.root",folder.Data()));
  ...
  if(OnlyCounts==1) return;
  // cout << "PORCA PALETTA" << endl;
  ...
  // TFile *Zana=new TFile(Form("%s/ZanalysisOnDATA.root",folder.Data()));
  // TFile *R_WdivZ=new TFile(Form("%s/R_WdivZ_OnMC.root",folder.Data()));
