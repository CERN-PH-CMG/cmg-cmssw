...
#include "PileupJetIdSubstructureNtupleAlgo.h"
...
class JetSubstructureAnalyzer : public edm::EDAnalyzer 
{
public:
  explicit JetSubstructureAnalyzer(const edm::ParameterSet&);
...
private:
  ...
  bool matchingToGenJet( const reco::PFJet jet, edm::View<reco::GenJet> genJets , float& genPt, float& genDr);
  void DiMuonSelection(edm::View<reco::Muon> muons, int& goodMuon1, int& goodMuon2, bool& isZcandidate);

  // ----------member data ---------------------------

  // PU jet identifier 
  PileupJetIdSubstructureNtupleAlgo *puIdAlgo_;
