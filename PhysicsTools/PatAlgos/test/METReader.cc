...
METReader::METReader(const edm::ParameterSet& iConfig) {
  ...
  _origMetLabel = iConfig.getParameter<edm::InputTag>("originalSlimmedMetlabel");
...
METReader::analyze(const Event& iEvent, 
  ...
  //extra Txy stuff =======================================================
  _newT1TxyPhi = t1txyMet[0].phi();
  _newT1TxyPt = t1txyMet[0].pt();
  ...
  if(_n<5) {
    ...
    std::cout<<"T1 TauUp   -> "<<_newT1TESUp<<std::endl;
    std::cout<<"T1 TauDo   -> "<<_newT1TESDo<<std::endl;
